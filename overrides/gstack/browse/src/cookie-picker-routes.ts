 ...
 *
 * Data/action routes require Bearer auth when the server provides an auth token.
 * Unit tests may call this handler directly with authToken undefined; in that case
 * the auth gate is bypassed so route logic can be tested in isolation.
 */
...
export async function handleCookiePickerRoute(
  ...
  try {
    ...
    // ─── Auth gate: all data/action routes below require Bearer token when configured ───
    const authHeader = req.headers.get('authorization');
    if (authToken && (!authHeader || authHeader !== `Bearer ${authToken}`)) {
      return jsonResponse({ error: 'Unauthorized' }, { port, status: 401 });
    }
    ...
    return errorResponse('Not found', 'not_found', { port, status: 404 });
  } catch (err: any) {
