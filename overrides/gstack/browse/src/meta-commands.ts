...
export function validateOutputPath(filePath: string): void {
  ...
  try {
    // Existing file/symlink: resolve the final target so symlink escapes are blocked.
    realResolved = fs.realpathSync(resolved);
  } catch {
    // New file path: resolve the parent directory to catch symlinked directories.
