#!/usr/bin/env bun
/**
 * gstack-global-discover — Discover AI coding sessions across pi, Codex CLI, and Gemini CLI.
...
interface Repo {
  ...
  remote: string;
  paths: string[];
  sessions: { pi_code: number; codex: number; gemini: number };
}
...
interface DiscoveryResult {
  ...
  repos: Repo[];
  tools: {
    pi_code: { total_sessions: number; repos: number };
...
function scanClaudeCode(since: Date): Session[] {
  const projectsDir = join(homedir(), ".pi", "projects");
  ...
  for (const dirName of dirs) {
    ...
    for (let i = 0; i < recentFiles.length; i++) {
      sessions.push({ tool: "pi_code", cwd });
    }
  }
...
async function resolveAndDeduplicate(sessions: Session[]): Promise<Repo[]> {
  ...
  for (const [remote, data] of byRemote) {
    ...
    const sessionCounts = { pi_code: 0, codex: 0, gemini: 0 };
  ...
  repos.sort(
    (a, b) =>
      b.sessions.pi_code + b.sessions.codex + b.sessions.gemini -
      (a.sessions.pi_code + a.sessions.codex + a.sessions.gemini)
  );
...
async function main() {
  ...
  // Summary to stderr
  console.error(
    `Discovered: ${ccSessions.length} pi sessions, ${codexSessions.length} Codex sessions, ${geminiSessions.length} Gemini sessions`
  );
  ...
  const ccRepos = new Set(repos.filter((r) => r.sessions.pi_code > 0).map((r) => r.remote)).size;
  ...
  const result: DiscoveryResult = {
    ...
    repos,
    tools: {
      pi_code: { total_sessions: ccSessions.length, repos: ccRepos },
  ...
  if (format === "json") {
    ...
    console.log(`Sessions: ${allSessions.length} total (pi: ${ccSessions.length}, Codex: ${codexSessions.length}, Gemini: ${geminiSessions.length})`);
    console.log(`Repos: ${repos.length} unique`);
    console.log("");
    for (const repo of repos) {
      const total = repo.sessions.pi_code + repo.sessions.codex + repo.sessions.gemini;
      const tools = [];
      if (repo.sessions.pi_code > 0) tools.push(`pi:${repo.sessions.pi_code}`);
