...
describe('gstack-team-init', () => {
  ...
  test('optional: creates AGENTS.md with recommended section', () => {
    const result = run(`${TEAM_INIT} optional`, { cwd: tmpDir });
    expect(result.exitCode).toBe(0);
    const claude = fs.readFileSync(path.join(tmpDir, 'AGENTS.md'), 'utf-8');
    expect(claude).toContain('## gstack (recommended)');
    expect(claude).toContain('./setup --team');
  });

  test('required: creates AGENTS.md with required section', () => {
  ...
  test('required: creates enforcement hook', () => {
    ...
    const hookPath = path.join(tmpDir, '.pi', 'hooks', 'check-gstack.sh');
  ...
  test('idempotent: running twice does not duplicate AGENTS.md section', () => {
  ...
  test('removes vendored copy when present', () => {
    ...
    const vendoredDir = path.join(tmpDir, '.pi', 'skills', 'gstack');
    ...
    // Track it in git
    execSync('git add .pi/skills/gstack/', { cwd: tmpDir });
    ...
    expect(gitignore).toContain('.pi/skills/gstack/');
  });
  ...
  test('skips when .pi/skills/gstack is a symlink', () => {
  ...
  test('does not duplicate .gitignore entry on re-run', () => {
    ...
    const gitignore = fs.readFileSync(path.join(tmpDir, '.gitignore'), 'utf-8');
    const matches = gitignore.match(/\.pi\/skills\/gstack\//g);
    expect(matches).toHaveLength(1);
  });
});

describe('setup --team / --no-team / -q', () => {
  test('setup -q suppresses the interactive naming prompt', () => {
    const result = run(`${path.join(ROOT, 'setup')} -q`, { cwd: ROOT });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).not.toContain('Skill naming:');
  });

  test('setup --local still installs project-local skills', () => {
    const result = run(`bash -c '${path.join(ROOT, 'setup')} --local -q 2>&1'`, { cwd: ROOT });
    expect(result.exitCode).toBe(0);
    expect(result.stdout).toContain('gstack ready (project-local).');
    expect(result.stdout).toContain('.pi/skills');
  });
});
