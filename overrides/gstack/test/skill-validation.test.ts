...
describe('v0.4.1 preamble features', () => {
  // Tier 1 skills have core preamble only (no ask the user in chat format)
  const tier1Skills = ['SKILL.md', 'browse/SKILL.md', 'setup-browser-cookies/SKILL.md', 'benchmark/SKILL.md'];

  // Tier 2+ skills have ask the user in chat format with RECOMMENDATION
  ...
  for (const skill of tier2PlusSkills) {
    test(`${skill} contains RECOMMENDATION format`, () => {
      ...
      expect(content).toContain('ask the user in chat');
    });
  }
...
describe('CEO review mode validation', () => {
  ...
  test('contains prerequisite skill offer for office-hours', () => {
    ...
    expect(content).toContain('/skill:office-hours');
  });
...
describe('Test Bootstrap ({{TEST_BOOTSTRAP}}) integration', () => {
  ...
  test('TEST_BOOTSTRAP does NOT appear in qa-only/SKILL.md', () => {
    ...
    expect(content).toContain('Run `/skill:qa` to bootstrap');
  });
...
describe('Phase 8e.5 regression test generation', () => {
  ...
  test('regression test includes full attribution comment format', () => {
    ...
    expect(content).toContain('// Found by /skill:qa on');
...
describe('Codex skill', () => {
  test('codex/SKILL.md exists and has pi-port frontmatter', () => {
    const content = fs.readFileSync(path.join(ROOT, 'codex', 'SKILL.md'), 'utf-8');
    expect(content).toContain('name: codex');
    expect(content).toContain('version: 1.0.0');
    // Pi ignores allowed-tools frontmatter, so the port strips it.
  ...
  test('adversarial review in /skill:review always runs both passes', () => {
  ...
  test('adversarial review in /skill:ship always runs both passes', () => {
    const content = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
    expect(content).toContain('Adversarial review (always-on)');
    expect(content).toContain('adversarial-review');
    expect(content).toContain('reasoning_effort="high"');
  ...
  test('scope drift detection in /skill:review and /skill:ship', () => {
    const reviewContent = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
    const shipContent = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
  ...
  test('codex integration in /skill:plan-eng-review offers plan critique', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('Codex');
    expect(content).toContain('codex exec');
  });

  test('/skill:review persists a review-log entry for ship readiness', () => {
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
...
describe('Codex skill validation', () => {
  ...
  // Discover all Claude skills with templates (except /skill:codex which is Claude-only)
  ...
  test('all skills (except /skill:codex) have both Claude and Codex variants', () => {
  ...
  test('/skill:codex skill is Claude-only — no Codex variant', () => {
