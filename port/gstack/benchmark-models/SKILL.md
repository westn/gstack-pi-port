...
description: |
  ...
  for this skill?" with data instead of vibes. Separate from /skill:benchmark, which
...
# /skill:benchmark-models — Cross-Model Skill Benchmark

You are running the `/skill:benchmark-models` workflow. Wraps the `gstack-model-benchmark` binary with an interactive flow that picks a prompt, confirms providers, previews auth, and runs the benchmark.

Different from `/skill:benchmark` — that skill measures web page performance (Core Web Vitals, load times). This skill measures AI model performance on gstack skills or arbitrary prompts.
...
```bash
BIN="$HOME/.pi/agent/skills/gstack/bin/gstack-model-benchmark"
[ -x "$BIN" ] || BIN=".pi/skills/gstack/bin/gstack-model-benchmark"
...
Use ask the user in chat with the preamble format:
...
If A: list top-level gstack skills that have SKILL.md files (from `find . -maxdepth 2 -name SKILL.md -not -path './.*'`), ask the user to pick one via a second ask the user in chat. Use the picked SKILL.md path as the prompt file.
...
If at least one is OK: ask the user in chat:
...
- **Options:**
  ...
  - B) Only Claude. Completeness: 6/10 (no cross-model signal — use /skill:ship's review for solo claude benchmarks instead).
...
```bash
[ -n "$pi provider credentials" ] || grep -q 'ANTHROPIC' "$HOME/.pi/.credentials.json" 2>/dev/null && echo "JUDGE_AVAILABLE" || echo "JUDGE_UNAVAILABLE"
```

If judge is available, ask the user in chat:
