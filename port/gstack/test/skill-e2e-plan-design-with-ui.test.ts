 ...
 * must reach a real skill numbered-option ask the user in chat (its first design-rating
 ...
 * .pi/plans/. Drive past permission dialogs. Wait for a numbered-
...
describeE2E('/plan-design-review with UI scope (gate)', () => {
  test(
    'reaches a real skill ask the user in chat (or plan_ready) without echoing the no-UI early-exit phrase',
    async () => {
      ...
      try {
        ...
        // plan content as a follow-up message. pi rejects slash
        ...
        while (Date.now() - start < budgetMs) {
          ...
          // Real skill ask the user in chat visible (not a permission dialog)?
          if (
