...
/** Skills that must exist as individual .pi/skills/{name}/SKILL.md files
 *  for pi's auto-discovery to treat them as invokable via Skill tool.
 ...
 *   - Per-skill SKILL.md files under .pi/skills/ (so Skill tool sees them)
 *   - Project AGENTS.md with explicit routing rules AND (optionally) the
 *     4.7 overlay content directly inlined so `pi --mode json -p` sees it
 *   - git init
 *
 *  `includeOverlay` controls whether the opus-4-7 nudges (Fan out, Literal,
 *  etc.) get inlined into AGENTS.md — this is the A/B axis for the fanout
 *  test. `pi --mode json -p` doesn't auto-load SKILL.md content, so AGENTS.md is
...
function mkEvalRoot(suffix: string, includeOverlay: boolean): string {
  ...
  // so we can inline it into AGENTS.md when includeOverlay is true.
  ...
  // Project AGENTS.md. Explicit routing rules so the agent reaches for
...
- Weekly retro, what did we ship → invoke retro`;
  ...
  fs.writeFileSync(path.join(tmp, 'AGENTS.md'), claudeMd);
