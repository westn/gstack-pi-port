 ...
 * - Codex output uses $GSTACK_BIN, not ~/.pi/... (host-aware paths)
...
describe('Writing Style preamble section', () => {
  ...
  test('Codex tier-2 preamble uses host-aware path (no .pi/)', () => {
    ...
    expect(explainLine).not.toMatch(/~\/\.pi\//);
  ...
  test('tier 2+ preamble composition note references ask the user in chat Format', () => {
    const out = generatePreamble(makeCtx('claude', 2));
    // The Writing Style section should explicitly compose with the existing Format section
    expect(out).toContain('ask the user in chat Format');
  });
