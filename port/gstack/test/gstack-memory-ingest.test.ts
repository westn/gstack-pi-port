 ...
 *
 * Covers the unit-testable internals: parseTranscriptJsonl (Codex + pi +
 ...
 *
 * E2E coverage (full --probe / --bulk on real ~/.pi/projects) lives in
...
function writeClaudeCodeSession(home: string, projectName: string, sessionId: string, content: string): string {
  const projectsDir = join(home, ".pi", "projects", projectName);
...
describe("internal: parseTranscriptJsonl + buildTranscriptPage shape", () => {
  it("parses a pi JSONL session", async () => {
    ...
    const home = makeTestHome();
    const projDir = join(home, ".pi", "projects", "tmp-foo");
  ...
  it("treats a truncated last line as partial (does not crash)", () => {
    const home = makeTestHome();
    const projDir = join(home, ".pi", "projects", "tmp-bar");
    mkdirSync(projDir, { recursive: true });
