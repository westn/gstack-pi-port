/**
 * ask the user in chat format-compliance smoke (gate, paid, real-PTY).
 *
 * Asserts: when /plan-ceo-review fires its first ask the user in chat in plan
 ...
 * (e.g., ask the user in chat tool truncates long prose, conductor renderer mangles
 ...
 *
 * Trigger choice: /plan-ceo-review fires its mode-selection ask the user in chat
...
describeE2E('ask the user in chat format compliance (gate)', () => {
  test(
    'first ask the user in chat from /plan-ceo-review contains all 7 mandated format elements',
    async () => {
      ...
      try {
        ...
        // Wait for a SKILL ask the user in chat. Strategy: poll the visible buffer until it
        // contains both a numbered-option list AND the format markers we
        // expect (ELI10 + Recommendation). When both are present, it IS a
        // real format-compliant ask the user in chat — not a permission dialog or trust
        // prompt.
        //
        ...
        while (Date.now() - start < budgetMs) {
          ...
          if (session.exited()) {
            throw new Error(
              `claude exited (code=${session.exitCode()}) before ask the user in chat rendered.\n` +
          ...
          // ask the user in chat above this line — the only ask the user in chat that can produce these
          ...
          // buffer means the ask the user in chat is currently rendered (not scrolled away).
          ...
          // don't accidentally grant a permission inside a real ask the user in chat).
          if (
            hasCursor &&
          ...
          // Real ask the user in chat check: cursor visible AND markers present anywhere in
        ...
        if (!askUserQuestionVisible) {
          throw new Error(
            `ask the user in chat not rendered within ${budgetMs}ms.\n` +
        ...
        if (gaps.length > 0) {
          ...
          throw new Error(
            `ask the user in chat format compliance FAILED — missing ${gaps.length} mandated field(s):\n` +
