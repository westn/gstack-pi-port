...
describe('runAgentSdkTest — canUseTool extension', () => {
  ...
  test('ask the user in chat is auto-added to allowedTools when canUseTool is supplied', async () => {
    freshSem();
    const stub: StubConfig = {
      streams: [[systemInit(), assistantTurn([{ type: 'text', text: 'ok' }]), resultSuccess()]],
      calls: [],
    ...
    await runAgentSdkTest({
      ...BASE_OPTS,
      allowedTools: ['Read', 'Grep'], // explicitly omits ask the user in chat
      queryProvider: makeStubProvider(stub),
      canUseTool: async (_toolName, input) => ({ behavior: 'allow', updatedInput: input }),
    });
    const opts = stub.calls[0]!.options!;
    expect(opts.allowedTools).toContain('ask the user in chat');
    expect(opts.tools).toContain('ask the user in chat');
  });

  test('ask the user in chat is NOT auto-added when canUseTool is absent', async () => {
    freshSem();
    const stub: StubConfig = {
      streams: [[systemInit(), assistantTurn([{ type: 'text', text: 'ok' }]), resultSuccess()]],
      calls: [],
    ...
    const opts = stub.calls[0]!.options!;
    expect(opts.allowedTools).not.toContain('ask the user in chat');
  });

  test('passThroughNonask the user in chat helper returns allow+updatedInput', async () => {
    const { passThroughNonask the user in chat } = await import('../test/helpers/agent-sdk-runner');
    const result = passThroughNonask the user in chat('Read', { file_path: '/tmp/x' });
