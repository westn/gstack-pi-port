 ...
 * circuits to plan_ready without firing ask the user in chat. Both 'asked' and
...
describeE2E('plan-design-review plan-mode smoke (gate)', () => {
  ...
  test('does not silently auto-decide when --disallowedTools ask the user in chat is set', async () => {
    const obs = await runPlanSkillObservation({
      skillName: 'plan-design-review',
      inPlanMode: true,
    ...
    if (
      ...
      throw new Error(
        `plan-design-review ask the user in chat-blocked regression: outcome=${obs.outcome}\n` +
          `summary: ${obs.summary}\n` +
          `elapsed: ${obs.elapsedMs}ms\n` +
          `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
      );
