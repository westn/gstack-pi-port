...
describe('gstack-uninstall', () => {
  ...
  describe('integration tests with mock layout', () => {
    ...
    beforeEach(() => {
      ...
      fs.mkdirSync(path.join(mockHome, '.pi', 'skills', 'gstack'), { recursive: true });
      fs.writeFileSync(path.join(mockHome, '.pi', 'skills', 'gstack', 'SKILL.md'), 'test');

      // Create per-skill symlinks (both old unprefixed and new prefixed)
      fs.symlinkSync('gstack/review', path.join(mockHome, '.pi', 'skills', 'review'));
      fs.symlinkSync('gstack/ship', path.join(mockHome, '.pi', 'skills', 'gstack-ship'));

      // Create a non-gstack symlink (should NOT be removed)
      fs.mkdirSync(path.join(mockHome, '.pi', 'skills', 'other-tool'), { recursive: true });
    ...
    test('--force removes global Claude skills and state', () => {
      const result = spawnSync('bash', [UNINSTALL, '--force'], {
        ...
        env: {
          ...process.env,
          HOME: mockHome,
          GSTACK_DIR: path.join(mockHome, '.pi', 'skills', 'gstack'),
      ...
      expect(fs.existsSync(path.join(mockHome, '.pi', 'skills', 'gstack'))).toBe(false);

      // Per-skill symlinks pointing into gstack/ should be removed
      expect(fs.existsSync(path.join(mockHome, '.pi', 'skills', 'review'))).toBe(false);
      expect(fs.existsSync(path.join(mockHome, '.pi', 'skills', 'gstack-ship'))).toBe(false);

      // Non-gstack tool should still exist
      expect(fs.existsSync(path.join(mockHome, '.pi', 'skills', 'other-tool'))).toBe(true);
