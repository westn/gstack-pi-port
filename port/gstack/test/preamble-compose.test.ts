 ...
 *
 * Asserts that the ask the user in chat Format section renders BEFORE the
...
describe('Preamble composition order', () => {
  test('ask the user in chat Format renders before Model-Specific Behavioral Patch (tier 2, claude)', () => {
    const out = generatePreamble(makeCtx('claude', 2, 'claude'));
    const formatIdx = out.indexOf('## User Question Format');
  ...
  test('ask the user in chat Format renders before Model-Specific Behavioral Patch (tier 2, opus-4-7)', () => {
  ...
  test('ask the user in chat Format renders before Model-Specific Behavioral Patch (tier 3)', () => {
  ...
  test('ask the user in chat Format renders before Model-Specific Behavioral Patch (codex host)', () => {
  ...
  test('tier 1 preamble does NOT include ask the user in chat Format (but MAY include overlay)', () => {
    const out = generatePreamble(makeCtx('claude', 1));
    expect(out).not.toContain('## User Question Format');
  });
});
