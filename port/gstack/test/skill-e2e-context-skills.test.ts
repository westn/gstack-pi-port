 ...
 *
 * These spawn `pi --mode json -p "/context-save ..."` with the Skill tool enabled
 * and the skill installed in the workdir's .pi/skills/. Unlike the
...
// Shared install helper: copy both skill files + bin scripts + routing AGENTS.md
// into a tmp workdir. Matches the pattern from skill-routing-e2e.test.ts so
// pi --mode json -p discovers the skills via .pi/skills/ auto-scan.
function setupWorkdir(suffix: string): { workDir: string; gstackHome: string; slug: string } {
  ...
  // Install skills into .pi/skills/ for pi --mode json -p auto-discovery.
  const skillsDir = path.join(workDir, '.pi', 'skills');
  ...
  // Routing AGENTS.md: explicit instruction to always use the Skill tool.
  fs.writeFileSync(path.join(workDir, 'AGENTS.md'), `# Project Instructions
...
- The skill files are at ./.pi/skills/context-save/SKILL.md and
  ./.pi/skills/context-restore/SKILL.md.
`);
...
describeIfSelected('Context Skills E2E (live-fire)', [
  ...
  testConcurrentIfSelected('context-save-routing', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-save wintermute progress. Invoke via the Skill tool. Do NOT ask the user in chat.`,
...
Do NOT ask the user in chat.`,
  ...
  testConcurrentIfSelected('context-restore-fragment-match', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-restore payments — load the saved context whose title contains "payments". Invoke via the Skill tool. Report what was loaded. Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 10,
  ...
  testConcurrentIfSelected('context-restore-empty-state', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-restore — there are no saved contexts yet. Invoke via the Skill tool. Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 8,
      allowedTools: ['Skill', 'Bash', 'Read', 'Grep', 'Glob'],
  ...
  testConcurrentIfSelected('context-restore-list-delegates', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-restore list. Invoke via the Skill tool. Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 8,
      allowedTools: ['Skill', 'Bash', 'Read', 'Grep', 'Glob'],
  ...
  testConcurrentIfSelected('context-restore-legacy-compat', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-restore — load the most recent saved context. Invoke via the Skill tool. Report the content of the loaded file. Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 8,
      allowedTools: ['Skill', 'Bash', 'Read', 'Grep', 'Glob'],
  ...
  testConcurrentIfSelected('context-save-list-current-branch', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-save list — list saved contexts for the CURRENT branch only (default, no --all). Invoke via the Skill tool. The current branch is "main". Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 10,
      allowedTools: ['Skill', 'Bash', 'Read', 'Grep', 'Glob'],
  ...
  testConcurrentIfSelected('context-save-list-all-branches', async () => {
    ...
    const result = await runSkillTest({
      prompt: `Run /context-save list --all — list saved contexts from ALL branches (not just the current one). Invoke via the Skill tool. Report the full list. Do NOT ask the user in chat.`,
      workingDirectory: workDir,
      env: { GSTACK_HOME: gstackHome },
      maxTurns: 10,
      allowedTools: ['Skill', 'Bash', 'Read', 'Grep', 'Glob'],
