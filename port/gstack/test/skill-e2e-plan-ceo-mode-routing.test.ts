 ...
 * ask the user in chat and the user picks HOLD SCOPE or SCOPE EXPANSION,
 ...
 *
 * Tier: periodic (not gate). Each run navigates 8-12 prior ask the user in chats (telemetry,
 * proactive, routing, vendoring, brain, office-hours, premise×3, approach)
 * before reaching Step 0F. At ~30s per ask the user in chat that's a 4-6 min navigation
...
/**
 * Navigate prior ask the user in chats by picking option 1 until we hit an ask the user in chat whose
 * options match one of the 4 mode names. Returns the option index
 * matching `targetMode`, with the buffer marker pointing AT that ask the user in chat.
 *
 * Throws if we don't reach the mode ask the user in chat within `maxNav` prior ask the user in chats or
 * the overall budget.
 */
async function navigateToModeask the user in chat(
  session: ClaudePtySession,
  since: number,
  targetMode: ModeCase['mode'],
  opts: { maxNav?: number; budgetMs?: number } = {},
): Promise<{ modeIndex: number; visibleAtMode: string }> {
  // /plan-ceo-review's mode ask the user in chat (Step 0F) sits behind several preamble
  ...
  while (Date.now() - start < budgetMs) {
    ...
    // Is THIS the mode ask the user in chat?
    if (opts.some(o => MODE_RE.test(o.label))) {
      const target = opts.find(o => o.label.toUpperCase().includes(targetMode));
      if (!target) {
        throw new Error(
          `Mode ask the user in chat rendered but target "${targetMode}" not in option labels:\n` +
    ...
    // Not the mode ask the user in chat — answer with option 1 (recommended) and continue.
    if (priorAnswered >= maxNav) {
      throw new Error(
        `Navigated ${maxNav} prior ask the user in chats without reaching the mode ask the user in chat. ` +
        `Last list:\n${opts.map(o => `  ${o.index}. ${o.label}`).join('\n')}`,
      );
    }
    priorAnswered++;
  ...
  throw new Error(`Mode ask the user in chat not reached within ${budgetMs}ms`);
}
    ...
    test(
      ...
      async () => {
        ...
        try {
          ...
          const { modeIndex } = await navigateToModeask the user in chat(session, since, c.mode);
          ...
          // Wait for downstream evidence: either next ask the user in chat or plan_ready or
          ...
          while (Date.now() - start < budgetMs) {
            ...
            if (
              ...
              // Plan-ready AND a follow-up ask the user in chat are both visible but
