...
describeE2E('plan-eng-review plan-mode smoke (gate)', () => {
  ...
  test('ask the user in chat surfaces when --disallowedTools ask the user in chat is set', async () => {
    ...
    if (
      ...
      throw new Error(
        `plan-eng-review ask the user in chat-blocked regression: outcome=${obs.outcome}\n` +
          `summary: ${obs.summary}\n` +
          `elapsed: ${obs.elapsedMs}ms\n` +
          `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
      );
    ...
    if (obs.outcome === 'plan_ready') {
      if (!obs.planFile || !planFileHasDecisionsSection(obs.planFile)) {
        throw new Error(
          `plan-eng-review ask the user in chat-blocked regression: plan_ready without a "## Decisions" section in ${obs.planFile ?? '<no plan file detected>'} — Step 0 was silently skipped.\n` +
            `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
        );
      }
    }
  ...
  // is seeded, the skill MUST fire ask the user in chat (or fall back to a
  ...
  test('STOP gate fires when seeded plan forces Step 0 findings', async () => {
    const obs = await runPlanSkillObservation({
      ...
      // must use mcp__*__ask the user in chat (outcome='asked') or fall back to
