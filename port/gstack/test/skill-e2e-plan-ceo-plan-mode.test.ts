 ...
 * call ExitPlanMode without ever firing ask the user in chat — the user had to
...
describeE2E('plan-ceo-review plan-mode smoke (gate)', () => {
    ...
    if (obs.outcome !== 'asked') {
        ...
        obs.outcome === 'plan_ready'
          ...
          : obs.outcome === 'timeout'
            ...
            : obs.outcome === 'silent_write'
              ? `Silent Write/Edit fired to an unsanctioned path before any ask the user in chat — also a Step 0 skip.`
      ...
      throw new Error(
        `plan-ceo-review smoke FAILED: outcome=${obs.outcome}\n` +
          ...
          `and the "One issue = one user question in chat call" rule under "CRITICAL RULE — ` +
  ...
  // via `ps` on the live Conductor claude process). Native ask the user in chat
  ...
  test('ask the user in chat surfaces when --disallowedTools ask the user in chat is set', async () => {
    ...
    if (
      ...
      throw new Error(
        `plan-ceo-review ask the user in chat-blocked regression: outcome=${obs.outcome}\n` +
          `summary: ${obs.summary}\n` +
          `elapsed: ${obs.elapsedMs}ms\n` +
          `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
      );
    ...
    if (obs.outcome === 'plan_ready') {
      if (!obs.planFile) {
        throw new Error(
          `plan-ceo-review ask the user in chat-blocked regression: outcome=plan_ready but no plan file path detected in TTY output. Cannot verify the model used the fallback flow.\n` +
            `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
        );
      }
      if (!planFileHasDecisionsSection(obs.planFile)) {
        throw new Error(
          `plan-ceo-review ask the user in chat-blocked regression: model wrote ${obs.planFile} without a "## Decisions" section. Step 0 was silently skipped.\n` +
            `--- evidence (last 2KB visible) ---\n${obs.evidence}`,
        );
      }
    }
