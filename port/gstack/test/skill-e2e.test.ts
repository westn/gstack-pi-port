...
Choose SELECTIVE EXPANSION mode. Skip any user-question prompts — this is non-interactive.
...
Proceed directly to the full review. Skip any user-question prompts — this is non-interactive.
Write your complete review directly to ${planDir}/review-output.md
...
describeIfSelected('gstack-upgrade E2E', ['gstack-upgrade-happy-path'], () => {
  ...
  beforeAll(() => {
    ...
    const mockGstack = path.join(upgradeDir, '.pi', 'skills', 'gstack');
...
Skip any user-question prompts — auto-approve the upgrade. Write a summary of what you did to stdout.

IMPORTANT: The install directory is at ./.pi/skills/gstack — use that exact path.`,
...
DO research what's out there before proposing — search for civic tech and government data platform designs. Skip the font preview page. Skip any user-question prompts — this is non-interactive.
...
Skip the preamble bash block. Skip any user-question prompts — this is non-interactive. Rate each design dimension 0-10 and explain what would make it a 10. Then EDIT plan.md to add the missing design decisions (interaction state table, empty states, responsive behavior, etc.).
...
Skip the preamble bash block. Skip any user-question prompts — this is non-interactive. Write your findings directly to stdout.
...
Do NOT ask the user in chat — for any ask the user in chat prompts, choose the RECOMMENDED option automatically.
