...
// agent behavior. See AGENTS.md (or CLAUDE.md) "E2E eval failure blame protocol" for details.
...
describeE2E('Base branch detection', () => {
  ...
  test('/skill:review detects base branch and diffs against it', async () => {
...
Write your findings to ${dir}/review-output.md`,
  ...
  test('/skill:ship Step 0-1 detects base branch without destructive actions', async () => {
...
- The diff stat against the base branch`,
  ...
  test('/skill:retro detects default branch for git queries', async () => {
...
`);
  ...
  test('Test 1: core flow produces valid DESIGN.md + AGENTS.md (or CLAUDE.md)', async () => {
    const result = await runSkillTest({
...
Write DESIGN.md and AGENTS.md (or CLAUDE.md) (or update it) in the working directory.`,
  ...
  test('Test 2: research integration uses WebSearch', async () => {
    ...
    try { fs.unlinkSync(path.join(designDir, 'AGENTS.md (or CLAUDE.md)')); } catch {}
