/**
 * /plan-ceo-review per-finding ask the user in chat count (periodic, paid, real-PTY).
 *
 * Asserts the load-bearing rule "One issue = one user question in chat call" by
...
import {
  ...
  type ask the user in chatFingerprint,
 ...
 */
function pickSkipInterview(fp: ask the user in chatFingerprint): number {
...
describeE2E('/plan-ceo-review per-finding ask the user in chat count (periodic)', () => {
  test(
    `5-finding plan emits ${FLOOR_DISTINCT}-${CEILING_DISTINCT} review-phase ask the user in chats`,
    async () => {
      try {
  ...
  test(
    `paired-finding positive control: ${N_PAIRED} related findings produce ${FLOOR_PAIRED}-${CEILING_PAIRED} ask the user in chats`,
    async () => {
      try {
