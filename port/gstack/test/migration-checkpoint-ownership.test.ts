...
function setupFakeGstackRoot(tmpHome: string): string {
  ...
  const gstackDir = path.join(tmpHome, '.pi', 'skills', 'gstack');
...
describe('migration v1.1.3.0 — checkpoint ownership guard', () => {
  ...
  test('scenario A: directory symlink into gstack → removed', () => {
    ...
    const skillsDir = path.join(tmpHome, '.pi', 'skills');
  ...
  test('scenario E: nothing to do → no-op exit 0 (idempotent)', () => {
    ...
    fs.rmSync(path.join(tmpHome, '.pi', 'skills', 'gstack', 'checkpoint'), { recursive: true, force: true });
  ...
  test('scenario F: gstack not installed → no-op exit 0', () => {
    // No ~/.pi/agent/skills/gstack/ at all. Also no checkpoint install.
    fs.mkdirSync(path.join(tmpHome, '.pi', 'skills'), { recursive: true });
