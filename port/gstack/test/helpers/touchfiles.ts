...
export const E2E_TOUCHFILES: Record<string, string[]> = {
  ...
  // ask the user in chat-blocked regression case (--disallowedTools ask the user in chat
  ...
  // v1.21+ ask the user in chat-blocked regression tests — Conductor launches
  // claude with `--disallowedTools ask the user in chat --permission-mode default`
  // (verified via `ps`); skills must still surface user-decisions through a
  // fallback path (mcp__conductor__ask the user in chat or plan-file flow) rather
  ...
  // Per-finding ask the user in chat count + review-report-at-bottom assertion.
  ...
  // ask the user in chat format regression (RECOMMENDATION + Completeness: N/10)
  ...
  // each one spawns pi --mode json -p and costs ~$0.20-$0.40. Collectively they
  ...
  // behavior under @anthropic-ai/claude-agent-sdk (closer to real pi
  // than `pi --mode json -p`). testNames in the file are template literals so the
...
export const E2E_TIERS: Record<string, 'gate' | 'periodic'> = {
  ...
  'plan-ceo-mode-routing':     'periodic',   // ~$3/run, deep navigation through 8-12 prior ask the user in chats
  ...
  // ask the user in chat format regression — periodic (Opus 4.7 non-deterministic benchmark)
  ...
  // Context skills live-fire — periodic (each test spawns pi --mode json -p, ~$0.20-$0.40)
