 ...
 *
 * This sits alongside session-runner.ts (which drives `pi --mode json -p` as a
 * subprocess) but runs the model via the published @anthropic-ai/claude-agent-sdk
 * instead. The SDK exposes the same harness primitives pi itself uses,
 * so overlay-driven behavior change is measured against a closer approximation
 * of real pi than the `pi --mode json -p` subprocess path provides.
 *
 ...
 *     project .pi/, and local hooks, and arms are no longer comparable.
...
export interface RunAgentSdkOptions {
   ...
   *   - { type:'preset', preset:'claude_code' } -> use pi default
   ...
   * one: it measures "does adding overlay text to the REAL pi system
   ...
   * to assert on ask the user in chat content silently pass without asserting.
   *
   * Callback contract matches the SDK: fires on every tool-use approval
   * request and on ask the user in chat invocations. For non-ask the user in chat
   * tools that tests don't care about, use `passThroughNonask the user in chat`
   * to auto-allow them.
   */
...
/**
 * Pass-through helper: auto-allows any tool_use that isn't ask the user in chat.
 * Most plan-mode handshake tests only care about the handshake ask the user in chat;
 * every other tool (Read, Grep, Bash, Write, Edit, ExitPlanMode) should just
 * run. Compose with a test-specific ask the user in chat handler:
 *
 *   canUseTool: async (toolName, input, options) => {
 *     if (toolName === 'ask the user in chat') {
 *       // custom assertions + canned answer
 *       return { behavior: 'allow', updatedInput: { questions: input.questions, answers: {...} } };
 *     }
 *     return passThroughNonask the user in chat(toolName, input);
 *   }
 */
export function passThroughNonask the user in chat(
  toolName: string,
...
export async function runAgentSdkTest(
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      // that want ask the user in chat interception without this flip would
      ...
      // When canUseTool is supplied, ensure ask the user in chat is in the allowed
      // tools list. Without it, Claude can't invoke ask the user in chat at all
      // and the callback never has a chance to fire on it.
      const baseTools = opts.allowedTools ?? ['Read', 'Glob', 'Grep', 'Bash'];
      const resolvedTools =
        hasCanUseTool && !baseTools.includes('ask the user in chat')
          ? [...baseTools, 'ask the user in chat']
          : baseTools;
      ...
      for await (const ev of q) {
        ...
        if (ev.type === 'system' && (ev as SDKSystemMessage).subtype === 'init') {
          systemInitVersion =
            (ev as SDKSystemMessage).pi_code_version ?? 'unknown';
 ...
 * expect the old `pi --mode json -p` output (extractToolSummary, etc) work unchanged.
 */
