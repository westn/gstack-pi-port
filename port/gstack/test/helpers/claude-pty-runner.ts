/**
 * Real-PTY runner for pi plan-mode E2E tests.
 *
 ...
 * because plan mode doesn't use the ask the user in chat tool — it uses its
 * own TTY-rendered native confirmation UI.
 *
 * Why this exists: the SDK harness intercepts `canUseTool` for
 * `ask the user in chat`. Claude in plan mode renders its "Ready to execute"
 * confirmation as a native option list (1-4 numbered options) without
 * invoking the ask the user in chat tool. The SDK never sees it. Real PTY
...
export function isTrustDialogVisible(visible: string): boolean {
  // Phrase pi prints. Stable across versions in this branch's range.
 ...
 * when it short-circuits an ask the user in chat via the question-tuning resolver
 ...
 *   - `ctrl-g to edit in VSCode · ~/.pi/plans/<name>.md`
 *
 ...
 *
 * Used by v1.22 ask the user in chat-blocked regression tests to read the plan
...
export function extractPlanFilePath(visible: string): string | null {
  ...
  const patterns: RegExp[] = [
    ...
    new RegExp(`·\\s*(${PATH_ANCHOR}\\S*\\.pi\\/plans\\/\\S+\\.md)`, 'i'),
    // Fallback: any path-anchored reference to a .pi/plans .md file.
    new RegExp(`(${PATH_ANCHOR}\\S*\\.pi\\/plans\\/[\\w-]+\\.md)`, 'i'),
  ];
 ...
 * supposed to gate on, even when ask the user in chat is --disallowedTools and
 ...
 * and `navigateToModeask the user in chat` in the routing test so tuning stays in sync.
 */
export const TAIL_SCAN_BYTES = 1500;

/**
 * Detect a pi permission dialog. These render as a numbered
 * option list (so isNumberedOptionListVisible matches them) but they
 * are NOT a skill's ask the user in chat — they're claude asking the user
 * whether to grant a tool/file permission. Tests that look for skill
 * ask the user in chats must explicitly skip these.
 *
 * The English phrases below are stable across recent pi
...
/** Detect any ask the user in chat-shaped numbered option list with cursor. */
...
export function parseNumberedOptions(
  ...
  // for the active ask the user in chat. Older numbered lists (e.g., a granted permission
 ...
 * to detect Step 0F mode-selection ask the user in chats, and by the upcoming
...
/**
 * Find the position of the first ask the user in chat-style numbered-option list
...
export function classifyVisible(
  ...
  opts?: {
    /**
     * When true, treat Write/Edit to `.pi/plans/*` BEFORE any
     * ask the user in chat render as `wrote_findings_before_asking` rather than
  ...
  // (a numbered prompt means a permission/ask the user in chat is gating the write,
  ...
  while ((m = writeRe.exec(visible)) !== null) {
    ...
    const isPlanWrite = target.includes('.pi/plans');
    ...
    if (opts?.strictPlanWrites && isPlanWrite) {
      if (auqRenderIdx < 0 || writePos < auqRenderIdx) {
        return {
          ...
          summary: `Write/Edit to ${target} fired before any ask the user in chat render`,
        };
      }
    ...
    if (!sanctioned && !isNumberedOptionListVisible(visible)) {
      return {
        ...
        summary: `Write/Edit to ${target} fired before any ask the user in chat`,
      };
    }
  }
  ...
  if (isAutoDecidedVisible(visible)) {
    return {
      ...
      summary:
        'skill auto-decided an ask the user in chat via the AUTO_DECIDE preamble (the user never saw the prompt)',
    };
  }
  ...
  if (isNumberedOptionListVisible(visible)) {
    ...
    return {
      outcome: 'asked',
      summary: 'skill fired a numbered-option prompt (ask the user in chat or routing-injection)',
    };
  }
  return null;
}
...
// ────────────────────────────────────────────────────────────────────────────
// Per-finding ask the user in chat count primitives (used by runPlanSkillCounting).
//
...
/**
 * Captured identity of an ask the user in chat — the rendered question text plus
 ...
 */
export interface ask the user in chatFingerprint {
...
export type Step0BoundaryPredicate = (
  answeredFingerprint: ask the user in chatFingerprint,
) => boolean;
...
/**
 * Stable hash for an ask the user in chat's identity — combines normalized prompt
...
export async function launchClaudePty(
  ...
  if (!claudePath) {
    throw new Error(
      'claude binary not found on PATH. Install: https://www.npmjs.com/package/@mariozechner/pi-coding-agent',
    );
  }
...
export interface PlanSkillObservation {
   ...
   *                     ask the user in chat or the routing-injection prompt)
   ...
   * v1.22 ask the user in chat-blocked regression tests verify the plan file
 ...
 *     without firing ask the user in chat because they short-circuit.
 *
 ...
 * the ask the user in chat tool — so canUseTool never fired and the assertion
...
export async function runPlanSkillObservation(opts: {
   ...
   *  ask the user in chat-blocked regression tests to pass
   *  `['--disallowedTools', 'ask the user in chat']` (the flag set Conductor
   *  uses to remove native ask the user in chat in favor of its MCP variant).
   ...
   * rendered ask the user in chat list).
   */
  env?: Record<string, string>;
  /**
   ...
   * ask the user in chat or fall back to a Decisions section. Without this,
...
// counts distinct review-phase ask the user in chat fingerprints. The actual
...
export interface PlanSkillCountObservation {
  ...
  /** Wall time (ms) until the outcome was decided. */
  elapsedMs: number;
  /** All distinct ask the user in chats observed, in observation order. */
  fingerprints: ask the user in chatFingerprint[];
 ...
 * ask the user in chats until a terminal signal fires.
 *
 * Flow:
 ...
 *      pi unless the skill defines them, so the plan goes as a
...
export async function runPlanSkillCounting(opts: {
   ...
   */
  firstAUQPick?: (fp: ask the user in chatFingerprint) => number;
  ...
  const fingerprints: ask the user in chatFingerprint[] = [];
    ...
    while (Date.now() - budgetStart < timeoutMs) {
      ...
      while ((m = writeRe.exec(visible)) !== null) {
        ...
        if (!sanctioned && !isNumberedOptionListVisible(visible)) {
          return snapshot(
            'silent_write',
            `Write/Edit to ${target} fired before any ask the user in chat`,
            visible,
          );
        }
      }
      ...
      const fp: ask the user in chatFingerprint = {
