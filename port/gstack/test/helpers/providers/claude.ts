...
/**
 * Claude adapter — wraps the `claude` CLI via pi --mode json -p.
 *
...
export class ClaudeAdapter implements ProviderAdapter {
  ...
  async available(): Promise<AvailabilityCheck> {
    ...
    // Auth sniff: ~/.pi/.credentials.json OR ANTHROPIC_API_KEY
    const credsPath = path.join(os.homedir(), '.pi', '.credentials.json');
  ...
  /**
   * Parse pi --mode json -p --output-format json output. Shape (as of 2026-04):
