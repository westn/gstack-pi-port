...
describe('isPermissionDialogVisible', () => {
  ...
  test('does NOT match a skill ask the user in chat list', () => {
    const sample = `
...
describe('classifyVisible (runtime path through the runner classifier)', () => {
  ...
  test('write to sanctioned path (.pi/plans) → returns null (allowed)', () => {
    const visible = `
      ⏺ Write(/Users/me/.pi/plans/some-plan.md)
      ⎿  Wrote 42 lines
    `;
  ...
  test('strictPlanWrites: AUQ first then plan write — write_pos > auq_pos → not flagged', () => {
    ...
    const visible = [
      ...
      '  2. Approach B',
      '',
      '⏺ Write(/Users/me/.pi/plans/draft.md)',
      '⎿  Wrote 42 lines',
    ].join('\n');
    const result = classifyVisible(visible, { strictPlanWrites: true });
    expect(result?.outcome).toBe('asked');
  ...
  test('strictPlanWrites: only a permission dialog visible → plan write still flagged', () => {
    ...
    const visible = `
      ...
      Edit to /Users/me/.pi/plans/some-plan.md
...
describe('Step0BoundaryPredicate per-skill', () => {
  ...
  function fp(promptSnippet: string, optionLabels: string[]): ask the user in chatFingerprint {
