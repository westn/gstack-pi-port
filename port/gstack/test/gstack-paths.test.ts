...
// helper from inside a bash block (`eval "$(~/.pi/agent/skills/gstack/bin/gstack-paths)"`)
...
describe('gstack-paths', () => {
  ...
  test('PLAN_ROOT chain: GSTACK_PLAN_DIR > CLAUDE_PLANS_DIR > HOME > CWD', () => {
    ...
    expect(run({ HOME: '/tmp/myhome' }).PLAN_ROOT).toBe('/tmp/myhome/.pi/plans');
    // CWD fallback only verifiable on POSIX — Git Bash auto-populates HOME.
    if (process.platform !== 'win32') {
      expect(run({ HOME: '' }).PLAN_ROOT).toBe('.pi/plans');
    }
  });
