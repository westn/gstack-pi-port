 ...
 * to prefer mcp__*__ask the user in chat variants and fall back to plan-file
 ...
 *
 * Spawn:
 *   claude --permission-mode plan --disallowedTools ask the user in chat
 *   /plan-ceo-review
 *
 * Expected:
...
describeE2E('AUTO_DECIDE opt-in preserved under Conductor flags (periodic)', () => {
  test('user-opted-in question still auto-decides when ask the user in chat is --disallowedTools', async () => {
    ...
    try {
      ...
      if (obs.outcome === 'asked') {
        throw new Error(
          `AUTO_DECIDE regression: the model surfaced an ask the user in chat despite the user's never-ask preference.\n` +
            `summary: ${obs.summary}\n` +
