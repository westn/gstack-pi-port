 ...
 *   - Registry completeness (every ask the user in chat pattern has an id)
...
// -----------------------------------------------------------------------
// Registry completeness — warn about SKILL.md.tmpl user-question prompts
// that don't appear to map to any registry entry.
//
// This is NOT a strict CI failure. Many ask the user in chat invocations are
...
describe('ask the user in chat template coverage (informational)', () => {
  test('count of templates using ask the user in chat is non-trivial', () => {
    const templates = findAllTemplates();
    const usingAsk = templates.filter((p) =>
      fs.readFileSync(p, 'utf-8').includes('ask the user in chat'),
    );
    // At the time of writing, ~35 templates reference ask the user in chat.
  ...
  test('tier 2+ skills include the Question Tuning section', async () => {
    ...
    const ctx = {
      ...
      paths: {
        ...
        designDir: '~/.pi/agent/skills/gstack/design/dist',
      },
      preambleTier: 2,
    };
