...
_EXPLAIN_LEVEL=$(~/.pi/agent/skills/gstack/bin/gstack-config get explain_level 2>/dev/null || echo "default")
if [ "$_EXPLAIN_LEVEL" != "default" ] && [ "$_EXPLAIN_LEVEL" != "terse" ]; then _EXPLAIN_LEVEL="default"; fi
echo "EXPLAIN_LEVEL: $_EXPLAIN_LEVEL"
_QUESTION_TUNING=$(~/.pi/agent/skills/gstack/bin/gstack-config get question_tuning 2>/dev/null || echo "false")
...
if [ -f "$_LEARN_FILE" ]; then
  ...
  if [ "$_LEARN_COUNT" -gt 5 ] 2>/dev/null; then
    ~/.pi/agent/skills/gstack/bin/gstack-learnings-search --limit 3 2>/dev/null || true
  fi
else
  echo "LEARNINGS: 0"
fi
~/.pi/agent/skills/gstack/bin/gstack-timeline-log '{"skill":"ship","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
_CHECKPOINT_MODE=$(~/.pi/agent/skills/gstack/bin/gstack-config get checkpoint_mode 2>/dev/null || echo "explicit")
_CHECKPOINT_PUSH=$(~/.pi/agent/skills/gstack/bin/gstack-config get checkpoint_push 2>/dev/null || echo "false")
...
If the user invokes a skill in plan mode, the skill takes precedence over generic plan mode behavior. **Treat the skill file as executable instructions, not reference.** Follow it step by step starting from Step 0; the first ask the user in chat is the workflow entering plan mode, not a violation of it. ask the user in chat (any variant — `mcp__*__ask the user in chat` or native; see "ask the user in chat Format → Tool resolution") satisfies plan mode's end-of-turn requirement. If no variant is callable, fall back to writing the decision brief into the plan file as a `## Decisions to confirm` section + ExitPlanMode — never silently auto-decide. At a STOP point, stop immediately. Do not continue the workflow or call ExitPlanMode there. Commands marked "PLAN MODE EXCEPTION — ALWAYS RUN" execute. Call ExitPlanMode only after the skill workflow completes, or if the user tells you to cancel the skill or leave plan mode.

If `PROACTIVE` is `"false"`, do not auto-invoke or proactively suggest skills. If a skill seems useful, ask: "I think /skillname might help here — want me to run it?"

If `SKILL_PREFIX` is `"true"`, suggest/invoke `/gstack-*` names. Disk paths stay `~/.pi/agent/skills/gstack/[skill-name]/SKILL.md`.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.pi/agent/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise ask the user in chat with 4 options, write snooze state if declined).

If output shows `JUST_UPGRADED <from> <to>`: print "Running gstack v{to} (just updated!)". If `SPAWNED_SESSION` is true, skip feature discovery.

Feature discovery, max one prompt per session:
- Missing `~/.pi/agent/skills/gstack/.feature-prompted-continuous-checkpoint`: ask the user in chat for Continuous checkpoint auto-commits. If accepted, run `~/.pi/agent/skills/gstack/bin/gstack-config set checkpoint_mode continuous`. Always touch marker.
- Missing `~/.pi/agent/skills/gstack/.feature-prompted-model-overlay`: inform "Model overlays are active. MODEL_OVERLAY shows the patch." Always touch marker.
...
If B: run `~/.pi/agent/skills/gstack/bin/gstack-config set explain_level terse`.
...
If `TEL_PROMPTED` is `no` AND `LAKE_INTRO` is `yes`: ask telemetry once via ask the user in chat:
...
> Let gstack proactively suggest skills, like /skill:qa for "does this work?" or /skill:investigate for bugs?
...
If B: run `~/.pi/agent/skills/gstack/bin/gstack-config set routing_declined true` and say they can re-enable with `gstack-config set routing_declined false`.

This only happens once per project. Skip if `HAS_ROUTING` is `yes` or `ROUTING_DECLINED` is `true`.

If `VENDORED_GSTACK` is `yes`, warn once via ask the user in chat unless `~/.gstack/.vendoring-warned-$SLUG` exists:
...
3. Run `~/.pi/agent/skills/gstack/bin/gstack-team-init required` (or `optional`)
...
- Do NOT ask the user in chat for interactive prompts. Auto-choose the recommended option.
...
"ask the user in chat" can resolve to two tools at runtime: the **host MCP variant** (e.g. `mcp__conductor__ask the user in chat` — appears in your tool list when the host registers it) or the **native** pi tool.

**Rule:** if any `mcp__*__ask the user in chat` variant is in your tool list, prefer it. Hosts may disable native AUQ via `--disallowedTools ask the user in chat` (Conductor does, by default) and route through their MCP variant; calling native there silently fails. Same questions/options shape; same decision-brief format applies.

**Fallback when neither variant is callable:** in plan mode, write the decision brief into the plan file as a `## Decisions to confirm` section + ExitPlanMode (the native "Ready to execute?" surfaces it). Outside plan mode, output the brief as prose and stop. **Never silently auto-decide** — only `/skill:plan-tune` AUTO_DECIDE opt-ins authorize auto-picking.

### Format

Every ask the user in chat is a decision brief and must be sent as tool_use, not prose.
...
Before calling ask the user in chat, verify:
- [ ] D<N> header present
...
_BRAIN_SYNC_BIN="~/.pi/agent/skills/gstack/bin/gstack-brain-sync"
_BRAIN_CONFIG_BIN="~/.pi/agent/skills/gstack/bin/gstack-config"

# /skill:sync-gbrain context-load: teach the agent to use gbrain when it's available.
# Mutually exclusive variants per /skill:plan-eng-review §4. Empty string when gbrain
...
if [ -f "$_GBRAIN_CONFIG" ] && command -v gbrain >/dev/null 2>&1; then
  ...
  if [ "$_GBRAIN_VERSION_OK" -gt 0 ] 2>/dev/null; then
    ...
    if [ "$_CWD_PAGES" -gt 0 ] 2>/dev/null; then
      ...
      echo "Run /skill:sync-gbrain to refresh."
    else
      echo "GBrain configured but this repo isn't indexed yet. Run \`/skill:sync-gbrain --full\`"
...
```bash
"~/.pi/agent/skills/gstack/bin/gstack-brain-sync" --discover-new 2>/dev/null || true
"~/.pi/agent/skills/gstack/bin/gstack-brain-sync" --once 2>/dev/null || true
```
...
**subordinate** to skill workflow, STOP points, ask the user in chat gates, plan-mode
safety, and /skill:ship review gates. If a nudge below conflicts with skill instructions,
...
Applies to ask the user in chat, user replies, and findings. ask the user in chat Format is structure; this is prose quality.
...
`/skill:context-restore` reads `[gstack-context]`; `/skill:ship` squashes WIP commits into clean commits.
...
If you are looping on the same diagnostic, same file, or failed fix variants, STOP and reassess. Consider escalation or /skill:context-save. Progress summaries must NEVER mutate git state.

## Question Tuning (skip entirely if `QUESTION_TUNING: false`)

Before each ask the user in chat, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `~/.pi/agent/skills/gstack/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /skill:plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
~/.pi/agent/skills/gstack/bin/gstack-question-log '{"skill":"ship","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"SKILL_NAME","type":"operational","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"observed"}'
```
...
In plan mode before ExitPlanMode: if the plan file lacks `## GSTACK REVIEW REPORT`, run `~/.pi/agent/skills/gstack/bin/gstack-review-read` and append the standard runs/status/findings table. With `NO_REVIEWS` or empty, append a 5-row placeholder with verdict "NO REVIEWS YET — run `/skill:autoplan`". If a richer report exists, skip.
...
You are running the `/skill:ship` workflow. This is a **non-interactive, fully automated** workflow. Do NOT ask for confirmation at any step. The user said `/skill:ship` which means DO IT. Run straight through and output the PR URL at the end.
...
Re-running `/skill:ship` means "run the whole checklist again." Every verification step
...
Never skip a verification step because a prior `/skill:ship` run already performed it.
...
Check diff size: `git diff <base>...HEAD --stat | tail -1`. If the diff is >200 lines, add: "Note: This is a large diff. Consider running `/skill:plan-eng-review` or `/skill:autoplan` for architecture-level review before shipping."

If CEO Review is missing, mention as informational ("CEO Review not run — recommended for product changes") but do NOT block.

For Design Review: run `source <(~/.pi/agent/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 9, but consider running /skill:design-review for a full visual audit post-implementation." Still never block.
...
3. **If no release pipeline exists and a new artifact was added:** Use ask the user in chat:
...
`/skill:ship` is a pre-merge gate, so always use full tier (Sonnet structural + Opus persona judges).
...
**Tier reference (for context — /skill:ship always uses `full`):**
| Tier | When | Speed (cached) | Cost |
|------|------|----------------|------|
| `fast` (Haiku) | Dev iteration, smoke tests | ~5s (14x faster) | ~$0.07/run |
| `standard` (Sonnet) | Default dev, `bin/test-lane --eval` | ~17s (4x faster) | ~$0.37/run |
| `full` (Opus persona) | **`/skill:ship` and pre-merge** | ~72s (baseline) | ~$1.27/run |
...
**IRON RULE:** When the coverage audit identifies a REGRESSION — code that previously worked but the diff broke — a regression test is written immediately. No ask the user in chat. No skipping. Regressions are the highest-priority test because they prove something broke.
...
- **>= minimum, < target:** Use ask the user in chat:
  ...
  - If A: Loop back to substep 5 (generate tests) targeting the remaining gaps. After second pass, if still below target, present ask the user in chat again with updated numbers. Maximum 2 generation passes total.
  - If B: Continue. Include in PR body: "Coverage gate: {X}% — user accepted risk."
  - If C: Continue. Include in PR body: "Coverage gate: {X}% — {N} paths intentionally uncovered."

- **< minimum:** Use ask the user in chat:
...
After producing the coverage diagram, write a test plan artifact so `/skill:qa` and `/skill:qa-only` can consume it:
...
**If the subagent fails, times out, or returns invalid JSON:** Fall back to running the audit inline in the parent. Do not block /skill:ship on subagent failure — partial results are better than none.
...
for PLAN_DIR in "$HOME/.gstack/projects/$_PLAN_SLUG" "$HOME/.pi/plans" "$HOME/.codex/plans" ".gstack/plans"; do
  [ -d "$PLAN_DIR" ] || continue
...
- **Any NOT DONE items:** Use ask the user in chat:
...
3. If `deferred > 0` and no user override, present the deferred items via ask the user in chat before continuing.
4. Embed `summary` in PR body's `## Plan Completion` section (Step 19).

**If the subagent fails or returns invalid JSON:** Fall back to running the audit inline. Never block /skill:ship on subagent failure.

---

## Step 8.1: Plan Verification

Automatically verify the plan's testing/verification steps using the `/skill:qa-only` skill.
...
Read the `/skill:qa-only` skill from disk:
...
- **Any FAIL:** Use ask the user in chat:
...
```bash
_CROSS_PROJ=$(~/.pi/agent/skills/gstack/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  ~/.pi/agent/skills/gstack/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  ~/.pi/agent/skills/gstack/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```

If `CROSS_PROJECT` is `unset` (first time): Use ask the user in chat:
...
If A: run `~/.pi/agent/skills/gstack/bin/gstack-config set cross_project_learnings true`
If B: run `~/.pi/agent/skills/gstack/bin/gstack-config set cross_project_learnings false`
...
1. Read `~/.pi/agent/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.pi/agent/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
```bash
source <(~/.pi/agent/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null) || true
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-specialist-stats 2>/dev/null || true
```
...
1. **Testing** — read `~/.pi/agent/skills/gstack/review/specialists/testing.md`
2. **Maintainability** — read `~/.pi/agent/skills/gstack/review/specialists/maintainability.md`
...
3. **Security** — if SCOPE_AUTH=true, OR if SCOPE_BACKEND=true AND DIFF_LINES > 100. Read `~/.pi/agent/skills/gstack/review/specialists/security.md`
4. **Performance** — if SCOPE_BACKEND=true OR SCOPE_FRONTEND=true. Read `~/.pi/agent/skills/gstack/review/specialists/performance.md`
5. **Data Migration** — if SCOPE_MIGRATIONS=true. Read `~/.pi/agent/skills/gstack/review/specialists/data-migration.md`
6. **API Contract** — if SCOPE_API=true. Read `~/.pi/agent/skills/gstack/review/specialists/api-contract.md`
7. **Design** — if SCOPE_FRONTEND=true. Use the existing design review checklist at `~/.pi/agent/skills/gstack/review/design-checklist.md`
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-search --type pitfall --query "{specialist domain}" --limit 5 2>/dev/null || true
```
...
1. The red-team checklist from `~/.pi/agent/skills/gstack/review/specialists/red-team.md`
...
6. **If ASK items remain,** present them in ONE ask the user in chat:
   - List each with number, severity, problem, recommended fix
   - Per-item options: A) Fix  B) Skip
   - Overall RECOMMENDATION
   - If 3 or fewer ASK items, you may use individual user question in chat calls instead

7. **After all fixes (auto + user-approved):**
   - If ANY fixes were applied: commit fixed files by name (`git add <fixed-files> && git commit -m "fix: pre-landing review fixes"`), then **STOP** and tell the user to run `/skill:ship` again to re-test.
...
and N values from the summary counts above. The `via:"ship"` distinguishes from standalone `/skill:review` runs.
...
> You are classifying Greptile review comments for a /skill:ship workflow. Read `~/.pi/agent/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps. Do NOT fix code, do NOT reply to comments, do NOT commit — report only.
>
...
**VALID & ACTIONABLE:** Use ask the user in chat with:
...
**VALID BUT ALREADY FIXED:** Reply using the **Already Fixed reply template** from greptile-triage.md — no ask the user in chat needed:
- Include what was done and the fixing commit SHA
- Save to both per-project and global greptile-history (type: already-fixed)

**FALSE POSITIVE:** Use ask the user in chat:
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"always","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"ship","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
if [ -f package.json ]; then
  ...
  if [ "$PARSE_EXIT" != "0" ]; then
    echo "ERROR: package.json is not valid JSON. Fix the file before re-running /skill:ship."
    exit 1
  fi
fi
...
if [ "$CURRENT_VERSION" = "$BASE_VERSION" ]; then
  if [ "$PKG_EXISTS" = "1" ] && [ -n "$PKG_VERSION" ] && [ "$PKG_VERSION" != "$CURRENT_VERSION" ]; then
    ...
    echo "This looks like a manual edit to package.json bypassing /skill:ship. Reconcile manually, then re-run."
    exit 1
  fi
  echo "STATE: FRESH"
else
...
if ! printf '%s' "$REPAIR_VERSION" | grep -qE '^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$'; then
  echo "ERROR: VERSION file contents ($REPAIR_VERSION) do not match MAJOR.MINOR.PATCH.MICRO pattern. Refusing to propagate invalid semver into package.json. Fix VERSION manually, then re-run /skill:ship."
  exit 1
fi
if command -v node >/dev/null 2>&1; then
  node -e 'const fs=require("fs"),p=require("./package.json");p.version=process.argv[1];fs.writeFileSync("package.json",JSON.stringify(p,null,2)+"\n")' "$REPAIR_VERSION" || {
...
Read `~/.pi/agent/skills/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use ask the user in chat:
...
**If disorganized** (missing priority fields, no component groupings, no Completed section): Use ask the user in chat:
...
user via ask the user in chat rather than destroying non-WIP commits.
...
Co-Authored-By: Claude Opus 4.7 <noreply@ai-assistant.local>
EOF
)"
```
...
> You are executing the /skill:document-release workflow after a code push. Read the full skill file `${HOME}/.pi/agent/skills/gstack/document-release/SKILL.md` and execute its complete workflow end-to-end, including CHANGELOG clobber protection, doc exclusions, risky-change gates, and named staging. Do NOT attempt to edit the PR body — no PR exists yet. Branch: `<branch>`, base: `<base>`.
>
...
**If the subagent fails or returns invalid JSON:** Print a warning and proceed to Step 19 without a `## Documentation` section. Do not block /skill:ship on subagent failure. The user can run `/skill:document-release` manually after the PR lands.
...
2. Compute the corrected title: `NEW_TITLE=$(~/.pi/agent/skills/gstack/bin/gstack-pr-title-rewrite.sh "$NEW_VERSION" "$CURRENT")`. The helper handles three cases: title already correct (no-op), title has a different `v<X.Y.Z.W>` prefix (replace it), or title has no version prefix (prepend one).
...
🤖 Generated with [pi](https://www.npmjs.com/package/@mariozechner/pi-coding-agent)
```
...
Log coverage and plan completion data so `/skill:retro` can track trends:
...
- **The goal is: user says `/skill:ship`, next thing they see is the review + PR URL + auto-synced docs.**
