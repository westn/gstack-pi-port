...
Log the operational learning now. Then say what you logged.`,
  ...
  testConcurrentIfSelected('session-awareness', async () => {
    ...
    // Extract ask the user in chat format instructions from a generated SKILL.md.
    // ROOT/SKILL.md is the browse skill (Tier 1) and does NOT contain the
    // "## User Question Format" section — that block is only emitted for
    ...
    if (!skillMd.includes('## User Question Format')) {
      ...
      for (const candidate of skillDirs) {
        ...
        if (content.includes('## User Question Format')) {
          skillMd = content;
