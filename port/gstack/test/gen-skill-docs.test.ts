...
describe('Codex generation (--host codex)', () => {
  ...
  test('sidecar paths point to .agents/skills/gstack/review/ (not gstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .pi/skills/review → .agents/skills/gstack-review
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.pi/agent/skills/gstack → $GSTACK_ROOT
    expect(content).not.toContain('~/.pi/agent/skills/gstack');
    expect(content).toContain('$GSTACK_ROOT');

    // Rule 2: .pi/skills/gstack → .agents/skills/gstack
    expect(content).not.toContain('.pi/skills/gstack');

    // Rule 3: .pi/skills/review → .agents/skills/gstack/review
    expect(content).not.toContain('.pi/skills/review');

    // Rule 4: .pi/skills → .agents/skills (catch-all)
    expect(content).not.toContain('.pi/skills');
  });
...
describe('Factory generation (--host factory)', () => {
  ...
  test('no .pi/skills/ in Factory output', () => {
    for (const skill of FACTORY_SKILLS) {
      const content = fs.readFileSync(path.join(FACTORY_DIR, skill.factoryName, 'SKILL.md'), 'utf-8');
      expect(content).not.toContain('.pi/skills');
    }
  ...
  test('no ~/.pi/agent/skills/ paths in Factory output', () => {
    for (const skill of FACTORY_SKILLS) {
      const content = fs.readFileSync(path.join(FACTORY_DIR, skill.factoryName, 'SKILL.md'), 'utf-8');
      // ~/.pi/agent/skills should be rewritten, but ~/.pi/plans is legitimate
      // (plan directory lookup) and ~/.pi/ in codex prompts is intentional
      expect(content).not.toContain('~/.pi/agent/skills');
    }
  });

  test('/skill:codex skill excluded from Factory output', () => {
