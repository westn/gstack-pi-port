...
describe('gen-skill-docs', () => {
  ...
  test('qa has fix-loop phases and pi-native frontmatter', () => {
    const qaContent = fs.readFileSync(path.join(ROOT, 'qa', 'SKILL.md'), 'utf-8');
    // Pi strips allowed-tools frontmatter, but the workflow sections must remain.
    expect(qaContent).not.toContain('allowed-tools:');
...
describe('REVIEW_DASHBOARD resolver', () => {
  ...
  test('plan-ceo-review chaining mentions eng and design reviews', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-ceo-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('/skill:plan-eng-review');
    expect(content).toContain('/skill:plan-design-review');
  });

  test('plan-eng-review chaining mentions design and ceo reviews', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('/skill:plan-design-review');
    expect(content).toContain('/skill:plan-ceo-review');
  });
  ...
  test('plan-design-review chaining mentions eng, ceo, and design skills', () => {
    ...
    expect(content).toContain('/skill:plan-eng-review');
    expect(content).toContain('/skill:plan-ceo-review');
    expect(content).toContain('/skill:design-shotgun');
    expect(content).toContain('/skill:design-html');
  });
...
describe('PLAN_FILE_REVIEW_REPORT resolver', () => {
  ...
  test('resolver output contains key report elements', () => {
    ...
    expect(content).toContain('/skill:plan-eng-review');
    expect(content).toContain('/skill:plan-design-review');
    expect(content).toContain('/skill:codex review');
  });
});
...
describe('Codex filesystem boundary', () => {
  ...
  const CODEX_CALLING_SKILLS = [
    'codex',         // /skill:codex skill — 3 modes
    'autoplan',      // /skill:autoplan — CEO/design/eng voices
    'review',        // /skill:review — adversarial step resolver
    'ship',          // /skill:ship — adversarial step resolver
  ...
  test('autoplan boundary text avoids host-specific paths for cross-host compatibility', () => {
    ...
    // paths like ~/.pi/ or .agents/skills (which break Codex/Claude output tests)
    ...
    expect(boundarySection).not.toContain('~/.pi/');
...
describe('INVOKE_SKILL resolver', () => {
  ...
  test('INVOKE_SKILL output includes default skip list', () => {
    ...
    expect(ceoContent).toContain('ask the user in chat Format');
  });
...
describe('preamble routing injection', () => {
  ...
  test('preamble bash checks for routing section in AGENTS.md', () => {
    expect(shipContent).toContain('grep -q "## Skill routing" AGENTS.md');
  ...
  test('preamble includes routing injection ask the user in chat', () => {
    expect(shipContent).toContain('Add routing rules to AGENTS.md');
...
describe('DESIGN_SKETCH extended with outside voices', () => {
  ...
  test('offers opt-in via ask the user in chat', () => {
...
describe('Codex generation (--host codex)', () => {
  ...
  // Dynamic discovery of expected Codex skills: all templates except /skill:codex
  ...
  const CODEX_SKILLS = (() => {
    ...
    for (const entry of fs.readdirSync(ROOT, { withFileTypes: true })) {
      if (!entry.isDirectory() || entry.name.startsWith('.') || entry.name === 'node_modules') continue;
      if (entry.name === 'codex') continue; // /skill:codex is excluded from Codex output
  ...
  test('no .pi/skills/ in Codex output', () => {
    for (const skill of CODEX_SKILLS) {
      const content = fs.readFileSync(path.join(AGENTS_DIR, skill.codexName, 'SKILL.md'), 'utf-8');
  ...
  test('no ~/.pi/ paths in Codex output', () => {
    for (const skill of CODEX_SKILLS) {
      const content = fs.readFileSync(path.join(AGENTS_DIR, skill.codexName, 'SKILL.md'), 'utf-8');
  ...
  test('/skill:codex skill excluded from Codex output', () => {
  ...
  test('sidecar paths point at the shared gstack review sidecar, not gstack-review/', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gstack-review', 'SKILL.md'), 'utf-8');
  ...
  test('greptile-triage sidecar path is correct', () => {
    ...
    if (content.includes('greptile-triage')) {
      expect(content).toMatch(/\$GSTACK_ROOT\/review\/greptile-triage\.md|\.agents\/skills\/gstack\/review\/greptile-triage\.md/);
      expect(content).not.toContain('.agents/skills/gstack-review/greptile-triage');
    }
  });

  test('Pi path rewrite rules produce correct Codex output', () => {
    const content = fs.readFileSync(path.join(AGENTS_DIR, 'gstack-review', 'SKILL.md'), 'utf-8');

    expect(content).not.toContain('~/.pi/agent/skills/gstack');
    expect(content).not.toContain('.pi/skills/gstack');
    expect(content).not.toContain('.pi/skills/review');
    expect(content).not.toContain('.pi/skills');
    expect(content).toContain('$GSTACK_ROOT');
  });
  ...
  test('path rewrite rules apply to all Codex skills with sidecar references', () => {
    ...
    for (const skill of CODEX_SKILLS) {
      ...
      expect(content).not.toContain('~/.pi/agent/skills');
      expect(content).not.toContain('.pi/skills');
  ...
  test('Claude output unchanged: review skill still uses .pi/skills/ paths', () => {
    // Codex changes must NOT affect Claude output
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.pi/agent/skills/review/checklist.md');
    expect(content).toContain('~/.pi/agent/skills/gstack');
    // Must NOT contain Codex paths
    expect(content).not.toContain('.agents/skills');
    expect(content).not.toContain('~/.codex/');
  });

  test('Claude output unchanged: ship skill still uses .pi/skills/ paths', () => {
    const content = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
    expect(content).toContain('~/.pi/agent/skills/gstack');
    expect(content).not.toContain('.agents/skills');
    expect(content).not.toContain('~/.codex/');
...
describe('--host all', () => {
  test('--host all generates for pi and codex', () => {
    ...
    expect(output).toContain('FRESH: .agents/skills/gstack/SKILL.md');
    expect(output).not.toContain('.factory/skills');
  });
});
...
describe('setup script validation', () => {
  ...
  test('pi install uses link_claude_skill_dirs', () => {
    // The Pi install section (section 4) should use the pi/legacy-Claude link function.
    const piSection = setupContent.slice(
      setupContent.indexOf('# 4. Install for pi'),
      setupContent.indexOf('# 5. Install for Codex')
    );
    expect(piSection).toContain('link_claude_skill_dirs');
    expect(piSection).not.toContain('link_codex_skill_dirs');
  });
  ...
  test('setup supports --host auto|pi|claude|codex|kiro', () => {
  ...
  // --- Pi-port setup semantics ---

  test('setup defaults to pi host and keeps claude as a compatibility alias', () => {
    expect(setupContent).toContain('HOST="pi"');
    expect(setupContent).toContain('[ "$HOST" = "pi" ] || [ "$HOST" = "claude" ]');
  });

  test('setup uses short skill names by default and supports --no-prefix', () => {
  ...
  test('welcome message references SKILL_PREFIX', () => {
    ...
    expect(setupContent).toContain('Run /skill:gstack-upgrade anytime');
  });
});
...
describe('LEARNINGS_SEARCH resolver', () => {
  ...
  test('learnings search includes ask the user in chat for first-time cross-project opt-in', () => {
    const content = fs.readFileSync(path.join(ROOT, 'review', 'SKILL.md'), 'utf-8');
