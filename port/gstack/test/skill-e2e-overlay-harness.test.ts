 ...
 * pi itself is built on. This complements test/skill-e2e-opus-47.test.ts
 * which measures the same thing via `pi --mode json -p` subprocess (a different
 ...
 *
 * Both arms have no AGENTS.md, no skills directory, no setting-source
  ...
  for (const fixture of OVERLAY_FIXTURES) {
    test(
      ...
      async () => {
        ...
        // Arm composition: both arms use the real pi default system
        ...
        // ON TOP of pi's normal behavioral scaffolding — which is
        // the only measurement that matches how real pi composes
