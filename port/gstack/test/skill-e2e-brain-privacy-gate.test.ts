 ...
 * every tool use via canUseTool, and asserts: one of the ask the user in chats
...
import { runAgentSdkTest, passThroughNonask the user in chat, resolveClaudeBinary } from './helpers/agent-sdk-runner';

const shouldRun = !!process.env.EVALS && process.env.EVALS_TIER === 'periodic';
const describeE2E = shouldRun ? describe : describe.skip;

describeE2E('gbrain-sync privacy gate fires once via preamble', () => {
  test('gstack skill preamble fires the 3-option ask the user in chat when gbrain is detected', async () => {
    ...
    try {
      ...
      await runAgentSdkTest({
        ...
        userPrompt:
          `Read the skill file at ${learnSkill} and follow its instructions from the top, including every preamble directive. Execute every bash block. If any ask the user in chat fires, present it.`,
  ...
  test('privacy gate does NOT fire when gbrain_sync_mode_prompted is already true', async () => {
    ...
    try {
      ...
      // No ask the user in chat should have matched the privacy gate's prose.
      const privacyQuestions = askUserQuestions.filter((aq) => {
        const qs = aq.input.questions as Array<{ question: string }>;
        return qs.some(
          (q) =>
