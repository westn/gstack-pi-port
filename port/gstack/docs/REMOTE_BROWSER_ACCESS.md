...
`/connect` is unauthenticated (rate-limited) — it's how a remote agent exchanges a setup key for a scoped session token. `/skill:health` is unauthenticated on the local listener (bootstrap) but does NOT exist on the tunnel listener (404).
...
|------|---------|------------|
| 401 | Token invalid, expired, or revoked | Ask user to run /skill:pair-agent again |
