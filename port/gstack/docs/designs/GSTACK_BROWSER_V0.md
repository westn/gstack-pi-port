...
with pi as the runtime and gives it a browser viewport.
...
sidebar extension baked in. You open it and pi can see your screen,
...
skill picker. Type "/skill:qa" to start QA testing, "/skill:investigate" to debug, "/skill:ship"
...
`pi --mode json -p` subprocess. BoomLooper provides:
...
**Today:** The sidebar chat connects to pi. You say "this button is
misaligned" and the AI reads the CSS, identifies the issue, and proposes a fix.
The `/skill:design-review` skill takes screenshots, identifies visual issues, and
...
|-------|-------------------|
| `/skill:qa` | Test every page, find bugs, fix them, verify fixes |
| `/skill:design-review` | Screenshot → analyze → fix CSS → screenshot again |
| `/skill:investigate` | See the error in browser → trace to code → fix → verify |
| `/skill:benchmark` | Measure page performance → detect regressions → alert |
| `/skill:canary` | Monitor deployed site → screenshot periodically → alert on changes |
| `/skill:ship` | Run tests → review diff → create PR → verify deployment in browser |
| `/skill:cso` | Audit page for XSS, open redirects, clickjacking in real browser |
| `/skill:office-hours` | Browse competitor sites → synthesize observations → design doc |
...
Generate mockup (GPT Image API)
  ...
  → Preview in browser
  → Fine-tune with /skill:design-review
  → Ship
```
...
+-----------┼────────────────────────────┼─────────────────+
  ...
  |  (HTTP + SSE)       |    |  (pi --mode json -p wrapper)  |
...
| Auth via `/skill:health` | **SHIPPED** | Replaces .auth.json file approach, auto-refreshes on server restart |
...
Local pi --mode json -p agent         Team workspaces                Cross-platform
...
1. **CEO Review** (`/skill:plan-ceo-review`, SELECTIVE EXPANSION) — 9 scope proposals,
   3 accepted (Cmd+K, Cmd+Shift+S, status bar), 5 deferred, 1 skipped
2. **Design Review** (`/skill:plan-design-review`) — scored 5/10 → 8/10, 9 design
   decisions added, 2 approved mockups generated
3. **Eng Review** (`/skill:plan-eng-review`) — 4 issues found, 0 critical gaps,
   test plan produced
