...
**Why tabled.** The v1 architecture assumed a pi `PostToolUse` hook could REPLACE the tool output that enters the model's context for built-in tools (Bash, Read, Grep, Glob, WebFetch). Research on 2026-04-17 confirmed this is not possible today.

**Evidence:**

1. **Official docs** (https://code.pi.com/docs/en/hooks): The only output-replace field documented for `PostToolUse` is `hookSpecificOutput.updatedMCPToolOutput`, and the docs explicitly state: *"For MCP tools only: replaces the tool's output with the provided value."* No equivalent field exists for built-in tools.
...
5. **Read/Grep/Glob execute in-process inside pi** and bypass hooks entirely. Wedge (ii) "native-tool coverage" was architecturally impossible from day one regardless of replacement API.
...
**Scope (Section 0):**
1. **Claude-first v1.** Ship compact + rules + verifier on pi only. Codex + OpenClaw land at v1.1 after the wedge is proven on the primary host. Cuts ~2 days of host integration and derisks launch. The original "wedge (ii) native-tool coverage" claim applies to pi at v1; we make no cross-host claim until v1.1.
...
**Current status: TABLED.** See "Status" section above. The architecture depends on a pi API (`updatedBuiltinToolOutput` or equivalent for built-in tools) that does not exist as of 2026-04-17. Anthropic issue #36843 tracks the gap.
...
2. ~~**Native-tool coverage.**~~ Architecturally impossible today. Read/Grep/Glob execute in-process inside pi and do not fire hooks. Even for tools that do fire `PostToolUse`, no output-replacement field exists for non-MCP tools.
...
| claude-token-efficient | 4.3K | MIT | Response verbosity | Different axis | Single AGENTS.md |
...
- No `pi provider credentials` → skip verifier, use pure rule output.
...
|------|-----------|-------------------|-------------|
| pi | `PostToolUse` | Bash, Read, Grep, Glob, Edit, Write, WebFetch, WebSearch, mcp__* | `~/.pi/settings.json` |
...
│   └── src/
│       ├── scanner.ts         # walk ~/.pi/projects/**/*.jsonl; pair tool_use × tool_result
...
| Cross-host E2E — pi only at v1 | free, ~1min | every PR (gate tier) | yes |
...
| ID | Scenario | Hosts |
|----|----------|-------|
| CH1 | Install hook via `gstack compact install <host>` | pi, Codex, OpenClaw |
...
| CH6 | Hook fires on Read tool | pi (confirmed); Codex/OpenClaw verify-then-require |
| CH7 | Hook fires on Grep tool | Same as CH6 |
| CH8 | Hook fires on Glob tool | Same as CH6 |
| CH9 | Hook fires on MCP tool (`mcp__*` matcher) | pi; verify on others |
...
| V6 | `pi provider credentials` missing | Verifier silently skipped; raw rule output returned |
...
```
┌──────────────────────────────────────────────────────────────┐
│  1. SCAN     scanner.ts walks ~/.pi/projects/**/*.jsonl  │
...
│              "On YOUR 30 days of pi data, gstack    │
...
- Reads `~/.pi/projects/**/*.jsonl` locally only. Never uploads. Never shares. Never logs scenarios to telemetry.
- Output files live under `~/.gstack/compact/benchmark/` with mode `0600`.
- The command prints a confirmation banner: *"Scanning local transcripts at ~/.pi/projects/ (local-only; nothing leaves this machine)."*
...
1. **Confirm the new API's shape.** Read the updated pi hooks reference. Capture a real envelope containing the new output-replacement field for Bash, Read, Grep, Glob. Record in `docs/designs/GCOMPACTION_envelope.md`.
...
3. **Re-run `/skill:plan-eng-review`** against the revised plan with the new API. Most of the 15 locked decisions should carry forward; adjust the Architecture data-flow and any envelope-dependent decisions.
4. **Re-run `/skill:codex review`** against the revised plan. The prior BLOCK verdict's concerns about hook substitution disappear once the API exists; remaining criticals (B-series privacy, regex DoS, JSON-envelope streaming) still apply.
...
2. **v0.1 (1 day):** pi hook integration + `gstack compact install` + mtime-based auto-reload. Ship as opt-in; off by default. Ask 10 gstack power users to try it; collect feedback.
...
| Platform compaction subsumes us (Anthropic Compaction API in pi) | Medium | We operate at a different layer (per-tool output vs whole-context). Position as complementary. |
...
6. **New:** Does pi's PostToolUse envelope include `exitCode`? (Still needs empirical verification per pre-implementation task #1; system now has a layered fallback regardless.)
...
1. **Verify pi's PostToolUse envelope contents empirically.** Ship a no-op hook; confirm `exitCode`, `command`, `argv`, `combinedText` are all present. This is the pivot for wedge (ii) native-tool coverage AND for the failureCompaction trigger. Output: `docs/designs/GCOMPACTION_envelope.md` with real captured envelopes for Bash + Read + Grep + Glob.
...
3. **Port analyze_transcripts JSONL parser to TypeScript.** `compact/benchmark/src/scanner.ts`. Write a quick-look output that lists the top-50 noisiest tool calls on the author's `~/.pi/projects/`. Confirms the testbench premise before we build the replay loop. This is the B-series foundation.
4. **Write the CHANGELOG entry FIRST.** Target sentence: *"Every tool in your agent's toolbox on pi now produces less noise — test runners, git diffs, package installs — with an intelligent Haiku safety net that restores critical stack frames when our rules over-compact, and a local benchmark that proves the savings on your actual 30 days of coding sessions. Codex + OpenClaw land in v1.1."* If we cannot write that sentence honestly, the wedge isn't there yet.
...
- [pi hooks reference](https://code.pi.com/docs/en/hooks)
...
- [Esteban Estrada: RTK 70% pi reduction](https://codestz.dev/experiments/rtk-rust-token-killer)
