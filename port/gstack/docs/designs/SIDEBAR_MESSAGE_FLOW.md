...
the `debug` toggle in the footer. The chat queue path (one-shot `pi --mode json -p`,
...
T+500ms   terminal-agent.ts boots
            ...
            ├── Writes <stateDir>/terminal-port (server reads it for /skill:health)
...
T+1-3s    Extension loads, sidebar opens
            ├── sidepanel-terminal.js: setState(IDLE), shows "Starting pi..."
...
|-------|----------|----------|----------|
| `AUTH_TOKEN` | `<stateDir>/skill:browse.json`; in-memory in server.ts | `/pty-session` POST (mint cookie + token) | server lifetime |
...
| State file | `<stateDir>/skill:browse.json` | Filesystem |
