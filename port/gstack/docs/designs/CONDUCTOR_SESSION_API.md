...
Server-Sent Events endpoint that re-emits pi's conversation as NDJSON events.

**Event types** (reuse pi's JSON mode event format):
...
- **Localhost-only.** Same trust model as pi's own debug output.
...
1. SSE endpoint that re-emits pi JSON mode events per workspace
2. `/api/workspaces` discovery endpoint with active workspace list
3. Content truncation (500 char cap on tool inputs/outputs)

Estimated effort: ~100-200 LOC if Conductor already captures the pi stream
...
| Auth | None (localhost) | Same as browse server, CDP port, pi |
