...
Today, `/skill:qa` and `/skill:design-review` feel like a black box. Claude says "I found 3 issues" but you can't see what it's looking at. With the sidebar connected to Conductor:
...
The only change on our side: swap the data source from "local `pi --mode json -p` subprocess" to "Conductor session stream." The extension code stays the same.
