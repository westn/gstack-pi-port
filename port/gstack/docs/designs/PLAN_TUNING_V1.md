...
A canonical record of what /skill:plan-tune v1 is, what it is NOT, what we considered, and why we made each call. Committed to the repo so future contributors (and future Garry) can trace reasoning without archeology. Supersedes any per-user local plan artifacts.
...
- /skill:plan-tune narrative + /skill:plan-tune vibe (V0 E3 item).
...
- **ELI10 as a new resolver file (`scripts/resolvers/eli10-writing.ts`).** Codex Pass 1 caught the conflict with existing "smart 16-year-old" framing in preamble's ask the user in chat Format section. Fold into existing preamble instead.
...
- **Acceptance criterion "≤ 12 ask the user in chat prompts per /skill:autoplan."** Removed from V1 — that target requires the pacing overhaul now in V1.1.
...
Generated SKILL.md body (static Markdown, baked at gen-skill-docs):
  - ask the user in chat Format section (existing V0)
...
- Pros: Modular.
- Cons (Codex #7): Conflicts with existing "smart 16-year-old" framing in preamble's ask the user in chat Format section. Two sources of truth.

**Extend preamble (chosen):** Writing Style section added to `scripts/resolvers/preamble.ts` directly below ask the user in chat Format.
...
Review report persisted in `~/.gstack/` via `gstack-review-log`. Plan file retained with full history at `~/.pi/plans/system-instruction-you-are-working-transient-sunbeam.md`.
