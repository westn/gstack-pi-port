...
**Authors:** garrytan (with /skill:plan-eng-review and /skill:codex outside-voice review)
...
| **2a** | `garrytan/browserharness` (continues) | `/skill:scrape <intent>` (read-only, single entry point with match/prototype paths) + `/skill:skillify` (codifies prototype into permanent skill). Adds `browse/src/browser-skill-write.ts` D3 atomic-write helper. **Shipping v1.19.0.0.** |
| **2b** | new (`browser-skills-automate`) | `/automate` skill template (mutating-flow sibling of `/skill:scrape`). Reuses `/skill:skillify` and the D3 helper. Per-mutating-step confirmation gate when running non-codified. P0 in TODOS. |
...
1. **Phase 1 = full storage + SDK + subcommands + bundled reference.** No agent
   authoring yet. Phase 2 lands `/skill:scrape` and `/automate`.
2. **Two verbs in Phase 2: `/skill:scrape` (read-only) and `/automate` (mutating).**
...
The /skill:codex review flagged 8 findings. The plan addresses them as follows:
...
## Phase 2a — `/skill:scrape` + `/skill:skillify` (shipping v1.19.0.0)

Two skill templates plus one helper module. `/skill:scrape <intent>` is the single
...
to a codified browser-skill in ~200ms. `/skill:skillify` codifies the most recent
...
### Decisions locked during the v1.19.0.0 plan review (`/skill:plan-eng-review`)

| ID | Decision | Locked behavior |
|----|----------|-----------------|
| **D1** | `/skill:skillify` provenance guard | Walk back ≤10 agent turns looking for a clearly-bounded `/skill:scrape` invocation (the prototype's intent line + its trailing JSON output). If not found, refuse with: *"No recent /skill:scrape result found in this conversation. Run /skill:scrape <intent> first, then say /skill:skillify."* No silent fallback. |
...
- **Default tier: global.** Lean global for procedures, with per-project
  override at `/skill:skillify` time (mirrors domain-skill scope). Phase 1 storage
...
- **Bun runtime distribution.** Codex finding #7 stays open. Phase 2a assumes
  ...
  in `/skill:skillify` SKILL.md "Limits". Real fix lands in Phase 4.

## Phase 2b — `/automate` sketch

Mutating-flow sibling of `/skill:scrape`. Same skillify pattern (reuses `/skill:skillify`
and the D3 helper as-is). Difference: per-mutating-step UNTRUSTED-wrapped
summary + `ask the user in chat` confirmation gate when run non-codified. After
...
nudges in `/skill:qa`, `/skill:design-review`, etc. fire when activity feed shows ≥N
