...
"failed .pi directory scan for skills"
...
4. `.pi/skills/` — pi compatibility fallback (lowest)
...
`/.slate/commands/`, `/.pi/commands/`, `/.agents/commands/`, `/.opencode/commands/`
...
Slate reads both `AGENTS.md` and `AGENTS.md` for project instructions.
Both literal strings confirmed in binary. No changes needed to existing
gstack projects... AGENTS.md works as-is.
...
```
--stream-json / --output-format stream-json  — JSONL output, "compatible with Anthropic pi SDK"
...
**Stream-JSON format:** Slate docs claim "compatible with Anthropic pi SDK."
Not yet empirically verified. Given OpenCode heritage, likely matches pi's
...
SLATE_DISABLE_CLAUDE_CODE                  — disable pi integration entirely
SLATE_DISABLE_CLAUDE_CODE_PROMPT           — disable pi prompt loading
SLATE_DISABLE_CLAUDE_CODE_SKILLS           — disable .pi/skills/ loading
...
**`SLATE_DISABLE_CLAUDE_CODE_SKILLS`** — When set, `.pi/skills/` loading is disabled.
...
gstack skills already work in Slate via the `.pi/skills/` fallback path.
No changes needed for basic functionality. Users who install gstack for pi
...
- E2E tests should set `SLATE_DISABLE_CLAUDE_CODE_SKILLS=1` to prove `.slate/` path
  actually works (not just falling back to `.pi/`)
...
- Parse: pi SDK-compatible NDJSON (assumed, needs verification)
- Skills: Install to `.slate/skills/` in test fixture (not `.pi/skills/`)
