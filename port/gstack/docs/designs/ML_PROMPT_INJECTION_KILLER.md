...
|--------|----------|--------|--------|
| pi Auto Mode | Two-layer: input probe scans tool outputs, transcript classifier (Sonnet 4.6, reasoning-blind) runs on every action | 0.4% FPR, 5.7% FNR | [Anthropic](https://www.anthropic.com/engineering/claude-code-auto-mode) |
...
**Key insights:**
- pi auto mode's transcript classifier is **reasoning-blind** by design. It
...
Note: The ask the user in chat happens through the Claude subprocess (which has access to
ask the user in chat), not through the extension UI (which doesn't have an ask-user primitive).
...
pi auto mode's input probe runs server-side on Anthropic's infrastructure.
...
- [pi Auto Mode](https://www.anthropic.com/engineering/claude-code-auto-mode)
- [pi Sandboxing](https://www.anthropic.com/engineering/claude-code-sandboxing)
