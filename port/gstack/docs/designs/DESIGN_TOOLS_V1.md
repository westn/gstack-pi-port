...
Generated by /skill:office-hours on 2026-03-26
...
A compiled TypeScript binary (`design/dist/design`) that wraps the OpenAI Images/Responses API, callable from skill templates via `$D` (mirroring the existing `$B` browse binary pattern). Priority integration order: /skill:office-hours → /skill:plan-design-review → /skill:design-consultation → /skill:design-review.
...
4. Priority: /skill:office-hours first, then /skill:plan-design-review, /skill:design-consultation, /skill:design-review.
...
- Scoped 48-hour prototype: shared `visual_mockup.ts` utility, /skill:office-hours + /skill:plan-design-review only, hero mockup + 2 variants
...
### Comparison Board Design Spec (from /skill:plan-design-review)
...
- Always show to user via the Read tool (which renders images inline in pi)
...
## CEO Expansion Scope (accepted via /skill:plan-ceo-review SCOPE EXPANSION)
...
- Used in: /skill:design-review, iteration feedback, PR review
...
- Bridge between /skill:design-review critique and visual fix proposal

### 4. Design Intent Verification
- During /skill:design-review, overlay approved mockup (docs/designs/) onto live screenshot
...
- Running `/skill:office-hours` on a UI idea produces actual PNG mockups alongside the design doc
- Running `/skill:plan-design-review` shows "what better looks like" as a mockup, not prose
...
- Symlinked via existing `~/.pi/agent/skills/gstack/` install path
...
### Commit 4: /skill:office-hours integration
...
### Commit 5: /skill:plan-design-review integration
...
- Integration into /skill:design-review template: auto-verify when approved mockup exists
...
- Integration into /skill:design-review: "Here's what the fix should look like" visual proposals
...
5. Running `/skill:office-hours` on a UI idea produces mockups inline
...
- You prioritized /skill:office-hours first — the upstream leverage point. If the brainstorm produces real mockups, every downstream skill (/skill:plan-design-review, /skill:design-review) has a visual artifact to reference instead of re-interpreting prose.
...
|--------|---------|-----|------|--------|----------|
| Office Hours | `/skill:office-hours` | Design brainstorm | 1 | DONE | 4 premises, 1 revised (Codex: opt-in->default-on) |
| CEO Review | `/skill:plan-ceo-review` | Scope & strategy | 1 | CLEAR | EXPANSION: 6 proposed, 6 accepted, 0 deferred |
| Eng Review | `/skill:plan-eng-review` | Architecture & tests (required) | 1 | CLEAR | 7 issues, 0 critical gaps, 4 outside voices |
| Design Review | `/skill:plan-design-review` | UI/UX gaps | 1 | CLEAR | score: 2/10 -> 8/10, 5 decisions made |
