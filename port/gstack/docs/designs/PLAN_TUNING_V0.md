...
1. **"Substrate" was false.** The plan wired 5 skills to read the profile on preamble, but ask the user in chat is a prompt convention, not middleware. Agents can silently skip the instructions. You cannot reliably build auto-decide on top of an unenforceable convention. Without a typed question registry that every ask the user in chat routes through, the substrate claim is marketing.
...
1. **Typed question registry** (`scripts/question-registry.ts`). Every ask the user in chat gstack uses is declared with `{id, skill, category, door_type, options[], signal_key?}`. Schema-governed.
2. **CI enforcement.** Lint test (gate tier) asserts every ask the user in chat pattern in SKILL.md.tmpl files has a matching registry entry. Fails CI on drift, renames, or duplicates.
...
5. **Preamble injection.** Before each ask the user in chat, agent calls `gstack-question-preference --check <registry-id>`. If `never-ask` AND question is NOT a one-way door, auto-choose recommended option with visible annotation: "Auto-decided [summary] → [option] (your preference). Change with /skill:plan-tune." One-way doors always ask regardless of preference — safety override.
...
7. **Declared profile** (`/skill:plan-tune setup`). 5 plain-English questions, one per dimension. Stored in unified `~/.gstack/developer-profile.json` under `declared: {...}`. Informational only in v1 — no skill behavior change.
...
9. **`/skill:plan-tune` skill.** Conversational plain-English inspection tool. "Show my profile," "set a preference," "what questions have I been asked," "show the gap between what I said and what I do." No CLI subcommand syntax required.
10. **Unification with existing `~/.gstack/builder-profile.jsonl`.** Fold /skill:office-hours session records and accumulated signals into unified `~/.gstack/developer-profile.json`. Migration is atomic + idempotent + archives the source file.
...
| E3 `/skill:plan-tune narrative` + `/skill:plan-tune vibe` | Event-anchored narrative needs stable profile. Without v1 data, output will be generic slop. | Profile diversity check passes for 2+ weeks real usage. Narrative test proves it quotes specific events, not clichés. |
...
| E5 LANDED celebration HTML page | Cannot live in preamble (Codex #9, #10). When promoted, moves to explicit command `/skill:plan-tune show-landed` OR post-ship hook — not passive detection in the hot path. | Explicit command or hook design. /skill:design-shotgun → /skill:design-html for the visual direction. Security + privacy review for PR data aggregation. |
| E6 Auto-adjustment based on mismatch | In v1, /skill:plan-tune shows the gap between declared and inferred. In v2, it could suggest declaration updates. Requires dual-track profile to be stable. | Real mismatch data from v1 shows consistent patterns. Suggestion UX designed separately. |
...
| TTHW telemetry for /skill:plan-tune onboarding | Codex #14. Contradicts local-first framing. Local logging only. |
...
~/.gstack/projects/{SLUG}/
  question-log.jsonl                # every ask the user in chat, append-only, registry-validated
...
**Diversity check** (Codex #13): `inferred` is considered "enough data" only when `sample_size >= 20 AND skills_covered >= 3 AND question_ids_covered >= 8 AND days_span >= 7`. Below this, `/skill:plan-tune profile` shows "not enough observed data yet" instead of a potentially-misleading inferred value.

## Data flow (v1)

1. Preamble: check `question_tuning` config. If off, do nothing.
2. Before each ask the user in chat:
...
- `/skill:plan-tune export <path>` writes profile to user-specified path (opt-in export).
- `/skill:plan-tune delete` wipes local profile files.
...
2. **Profile dimensions are inspectable AND editable.** `/skill:plan-tune profile` shows declared + inferred + gap. Edits via plain English go to `declared` only. System tracks `inferred` independently.
...
- Auditable: `/skill:plan-tune profile --trace autonomy` shows every event that contributed to the value.
...
(From /skill:plan-devex-review, user correction on CLI syntax):

`/skill:plan-tune` (no args) enters conversational mode. No CLI subcommand syntax required.
...
### Core schema
- `scripts/question-registry.ts` — typed registry. Seeded from audit of all SKILL.md.tmpl ask the user in chat invocations.
...
### Decision E: CLI subcommand structure for /skill:plan-tune — ANSWER: PLAIN ENGLISH CONVERSATIONAL (no subcommand syntax required)

**`/skill:plan-tune profile`, `/skill:plan-tune profile set autonomy 0.4`, etc.** (original):
...
**Plain-English conversational (revised after user correction):** `/skill:plan-tune` enters a menu. User says what they want in natural language.
...
**Explicit command (`/skill:plan-tune show-landed`):** User opts in.
...
**Post-ship hook (`/skill:ship` triggers detection after PR creation):** Tied to /skill:ship.
- Pros: Natural timing. No preamble cost.
- Cons: /skill:ship isn't always the landing event (manual merges, team members merging, etc.).
...
- Cons: Requires auditing every existing ask the user in chat in gstack — substantial up-front work.
...
- Every ask the user in chat invocation in every SKILL.md.tmpl has a registry entry. CI lint enforces.
...
- Dogfood: Garry uses `/skill:plan-tune` for 2+ weeks. Reports back whether:
...
1. Audit every `ask the user in chat` invocation in every gstack SKILL.md.tmpl. Build initial `scripts/question-registry.ts` with IDs, categories, door_types, options. This is the foundation; everything else sits on it.
...
5. Seed `scripts/archetypes.ts` with archetype definitions (referenced by future v2 `/skill:plan-tune vibe`).
...
17. `/skill:ship` v1. v2 scope discussion after dogfood.
...
5. Should /skill:plan-tune support a "team profile" mode where a shared developer-profile informs collaboration? v2+.
