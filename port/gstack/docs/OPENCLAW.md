...
OpenClaw's ACP runtime spawns pi sessions natively. gstack provides the
...
```
  ...
  calendar, memory, EA                     methodology + planning
       ...
       │       │                                │
       │       └── pi                  ├── Generates gstack-full
       │           └── gstack installed at      │   (complete pipeline)
       │               ~/.pi/agent/skills/gstack  │
...
| **Medium** | Multi-file features, refactors | gstack-lite AGENTS.md appended |
...
| **Plan** | "Help me plan a pi project" | gstack-plan pipeline appended |
...
- Does the user want to PLAN something for pi without implementing yet? -> **Plan**
...
1. **Always spawn, never redirect.** When the user asks to use ANY gstack skill,
   ALWAYS spawn a pi session. Never tell the user to open pi.
...
### AGENTS.md collision handling

When spawning pi in a repo that already has a AGENTS.md, APPEND
...
`openclaw/gstack-lite-AGENTS.md` — ~15 lines of planning discipline:
...
`openclaw/gstack-full-AGENTS.md` — chains existing gstack skills:
1. Read AGENTS.md and understand the project
2. Run /skill:autoplan (CEO + eng + design review)
3. Implement the approved plan
4. Run /skill:ship to create a PR
5. Report back with PR URL and decisions

### gstack-plan (Plan tier)
`openclaw/gstack-plan-AGENTS.md` — full review gauntlet, no implementation:
1. Run /skill:office-hours to produce a design doc
2. Run /skill:autoplan (CEO + eng + design + DX reviews + codex adversarial)
...
When pi runs inside a session spawned by OpenClaw, the `OPENCLAW_SESSION`
...
The agent should:
1. Install gstack-lite AGENTS.md into its coding session templates
...
- No full skill porting (coding skills stay native to pi)
