...
**Trust warning:** Pasting this URL gives your local pi full read/write access to every page in the shared brain. If that's not the trust level you want, pick PGLite local (Path 3) instead and accept the brains are disjoint.
...
**If you Ctrl-C mid-provision:** The SIGINT trap prints your in-flight project ref + a resume command. You can delete the orphan at the Supabase dashboard, or run `/skill:setup-gbrain --resume-provision <ref>` to pick up where you left off.
...
This is the best first choice if you just want to see what gbrain feels like before committing to cloud. You can always migrate later with `/skill:setup-gbrain --switch`.

## MCP registration for pi

By default the skill asks "Give pi a typed tool surface for gbrain?" If you say yes, it runs:

```bash
claude mcp add gbrain -- gbrain serve
```

That registers gbrain's stdio MCP server with pi. Now `gbrain search`, `gbrain put_page`, `gbrain get_page`, etc. show up as first-class tools in every session, not bash shell-outs.
...
```bash
/skill:setup-gbrain --repo      # re-prompt for this repo only

# Or directly:
~/.pi/agent/skills/gstack/bin/gstack-gbrain-repo-policy set "github.com/foo/bar" read-only
```

**To see every policy:**

```bash
~/.pi/agent/skills/gstack/bin/gstack-gbrain-repo-policy list
```
...
`/skill:setup-gbrain` offers to wire this up for you at the end of initial setup — it's one more ask the user in chat, and it integrates with the same private-repo infrastructure.
...
```bash
/skill:setup-gbrain --cleanup-orphans
```
...
### `/skill:setup-gbrain` entry modes

| Invocation | What it does |
|---|---|
| `/skill:setup-gbrain` | Full flow: detect state, pick path, install, init, MCP, policy, optional memory-sync |
| `/skill:setup-gbrain --repo` | Flip the per-remote trust policy for the current repo only |
| `/skill:setup-gbrain --switch` | Migrate engine (PGLite ↔ Supabase) without re-running the other steps |
| `/skill:setup-gbrain --resume-provision <ref>` | Resume a path-2a auto-provision that was interrupted during polling |
| `/skill:setup-gbrain --cleanup-orphans` | List + per-project delete of orphan Supabase projects |
...
| `~/.gstack/.brain-last-push` | Timestamp of last sync push (for `/skill:health` scoring) |
...
Then re-run `/skill:setup-gbrain`.
...
```bash
/skill:setup-gbrain --resume-provision <ref>
```

The skill re-collects a PAT, skips project creation, resumes polling.

### "Another `/skill:setup-gbrain` instance is running"
...
`/skill:health` treats that as yellow, not red. Check `gbrain doctor --json | jq .checks` to see which sub-checks are warning. Typical causes: resolver MECE overlap (skill names clashing) or DB connection not yet configured.

### Switching PGLite → Supabase hangs

Another gstack session in a sibling Conductor workspace may be holding a lock on your local PGLite file via its preamble's `gstack-brain-sync` call. Close other workspaces, re-run `/skill:setup-gbrain --switch`. The timeout is bounded at 180s so you'll never actually wait forever.
...
**Why not auto-import every repo?** Privacy + noise. An auto-import preamble hook that ingests every repo you touch would: (a) leak work code into a shared brain without consent, and (b) clog search with throwaway repos. The per-remote policy makes ingestion an explicit, per-repo decision. `/skill:setup-gbrain` doesn't install any auto-import hook today — but the policy store is forward-compatible for one later.

## Related skills + next steps

- `/skill:health` — includes a GBrain dimension (doctor status, sync queue depth, last-push age) in its 0-10 composite score. The dimension is omitted when gbrain isn't installed; running `/skill:health` on a non-gbrain machine doesn't penalize that choice.
- `/skill:gstack-upgrade` — keeps gstack itself up to date. Does NOT upgrade gbrain independently. To bump gbrain, update `PINNED_COMMIT` in `bin/gstack-gbrain-install` and re-run `/skill:setup-gbrain`.
- `/skill:retro` — weekly retrospective pulls learnings and plans from your gbrain when memory sync is on, letting the retro reference cross-machine history.

Run `/skill:setup-gbrain` and see what sticks.
