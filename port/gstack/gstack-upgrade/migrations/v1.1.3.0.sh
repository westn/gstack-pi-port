...
#
# pi ships /checkpoint as a native alias for /rewind, which was
...
# install so pi's native /checkpoint is no longer shadowed.
#
...
# ~/.pi/agent/skills/gstack/. A user's own /checkpoint skill (regular file,
# or symlink pointing elsewhere) is preserved.
#
# Three supported install shapes to handle:
#   1. ~/.pi/agent/skills/checkpoint is a directory symlink into gstack.
#   2. ~/.pi/agent/skills/checkpoint is a regular directory whose ONLY file
...
# --- Shape 1: top-level ~/.pi/agent/skills/checkpoint
if [ -L "$OLD_TOPLEVEL" ]; then
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    rm -- "$OLD_TOPLEVEL"
    echo "  [v1.1.3.0] Removed stale /checkpoint symlink (was shadowing pi's /rewind alias)."
    removed_any=1
  else
...
# --- Shape 2: ~/.pi/agent/skills/gstack/checkpoint/
...
if [ -L "$OLD_NAMESPACED" ]; then
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    rm -- "$OLD_NAMESPACED"
    echo "  [v1.1.3.0] Removed stale ~/.pi/agent/skills/gstack/checkpoint symlink."
    removed_any=1
  else
  ...
  if [ -n "$GSTACK_ROOT_REAL" ] && path_inside "$target_real" "$GSTACK_ROOT_REAL"; then
    ...
    echo "  [v1.1.3.0] Removed stale ~/.pi/agent/skills/gstack/checkpoint/ (replaced by context-save + context-restore)."
    removed_any=1
  else
...
if [ "$removed_any" = "1" ]; then
  echo "  [v1.1.3.0] /checkpoint is now pi's native /rewind alias. Use /context-save to save state and /context-restore to resume."
fi
