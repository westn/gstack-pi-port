...
6. **If CRITICAL issues found:** For EACH critical issue, use a separate ask the user in chat with:
...
- **Never ask for confirmation** except for MINOR/MAJOR version bumps and CRITICAL review findings (one ask the user in chat per critical issue with fix recommendation).
