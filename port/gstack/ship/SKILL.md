...
2. **If no override exists,** ask the user in chat:
   ...
   - Options: A) Ship anyway  B) Abort — run /skill:plan-eng-review first  C) Change is too small to need eng review
...
3. **If the user chooses A or C,** persist the decision so future `/skill:ship` runs on this branch skip the gate:
   ```bash
   eval "$(~/.pi/agent/skills/gstack/bin/gstack-slug 2>/dev/null)"
