...
You are running the `/skill:codex` skill. This wraps the OpenAI Codex CLI to get an independent,
...
1. `/skill:codex review` or `/skill:codex review <instructions>` — **Review mode** (Step 2A)
2. `/skill:codex challenge` or `/skill:codex challenge <focus>` — **Challenge mode** (Step 2B)
3. `/skill:codex` with no arguments — **Auto-detect:**
   - Check for a diff (with fallback if origin isn't available):
     `git diff origin/<base> --stat 2>/dev/null | tail -1 || git diff <base> --stat 2>/dev/null | tail -1`
   - If a diff exists, ask the user in chat:
     ```
...
4. `/skill:codex <anything else>` — **Consult mode** (Step 2C), where the remaining text is the prompt
...
CROSS-MODEL ANALYSIS:
  ...
  Only Claude found: [findings unique to Claude's /skill:review]
  Agreement rate: X% (N/M total unique findings overlap)
```

7. Persist the review result:
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
If a session file exists (not `NO_SESSION`), ask the user in chat:
```
...
root (`-C`) and cannot access `~/.pi/plans/` or any files outside the repo. You MUST
...
════════════════════════════════════════════════════════════
Tokens: N | Est. cost: ~$X.XX
Session saved — run /skill:codex again to continue this conversation.
```
...
agentic coding model). This means as OpenAI ships newer models, /skill:codex automatically
...
(e.g., `/skill:codex review --xhigh`) when they want maximum reasoning and are willing to wait.

**Web search:** All codex commands use `--enable web_search_cached` so Codex can look up
docs and APIs during review. This is OpenAI's cached index — fast, no extra cost.

If the user specifies a model (e.g., `/skill:codex review -m gpt-5.1-codex-max`
or `/skill:codex challenge -m gpt-5.2`), pass the `-m` flag through to codex.
