...
description: |
  ...
  fix evidence, and a ship-readiness summary. For report-only mode, use /skill:qa-only. (gstack)
voice-triggers:
  - "quality check"
  - "test the app"
  - "run QA"
...
"Your working tree has uncommitted changes. /skill:qa needs a clean tree so each bug fix gets its own atomic commit."
...
1. **Project-scoped test plans:** Check `~/.gstack/projects/` for recent `*-test-plan-*.md` files for this repo
   ...
   ```
2. **Conversation context:** Check if a prior `/skill:plan-eng-review` or `/skill:plan-ceo-review` produced test plan output in this conversation
...
- Include full attribution comment:
  ...
  // Found by /skill:qa on {YYYY-MM-DD}
...
2. **Fixed bugs that were in TODOS.md** → annotate with "Fixed by /skill:qa on {branch}, {date}"
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
