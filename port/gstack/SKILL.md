...
description: |
  ...
  elements, verify UI state, diff before/after actions, and capture annotated screenshots.
  Use it to test flows, verify deploys, and file bug reports with evidence.
  Suggest adjacent gstack skills by stage when useful: brainstorm (/skill:office-hours),
  strategy (/skill:plan-ceo-review), architecture (/skill:plan-eng-review), design planning
  (/skill:plan-design-review or /skill:design-consultation), auto-review (/skill:autoplan),
  debugging (/skill:investigate), QA (/skill:qa), code review (/skill:review), visual audit
  (/skill:design-review), shipping (/skill:ship), docs (/skill:document-release), retro
  (/skill:retro), second opinion (/skill:codex), safety (/skill:careful or /skill:guard),
  scoped edits (/skill:freeze or /skill:unfreeze), and upgrades (/skill:gstack-upgrade).
  If user opts out, run `gstack-config set proactive false`; opt back in with `... true`.
---
