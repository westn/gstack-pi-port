...
Ask these questions **ONE AT A TIME** via ask the user in chat. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via ask the user in chat. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via ask the user in chat: "Should we build on this prior design or start fresh?"
...
After understanding the problem through questioning, search for what the world thinks. This is NOT competitive research (that's /skill:design-consultation's job). This is understanding conventional wisdom so you can evaluate where it's wrong.

**Privacy gate:** Before searching, ask the user in chat: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
Options: A) Yes, search away  B) Skip — keep this session private
If B: skip this phase entirely and proceed to Phase 3. Use only in-distribution knowledge.

When searching, use **generalized category terms** — never the user's specific product name, proprietary concept, or stealth idea. For example, search "task management app landscape" not "SuperTodo AI-powered task killer."

If no external search extension is installed, skip this phase and note: "Search unavailable — proceeding with in-distribution knowledge only."

**Startup mode:** If an external search extension is installed, search for:
- "[problem space] startup approach {current year}"
- "[problem space] common mistakes"
- "why [incumbent solution] fails" OR "why [incumbent solution] works"

**Builder mode:** If an external search extension is installed, search for:
...
Use ask the user in chat to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Use ask the user in chat (regardless of codex availability):
...
If `$D serve` is not available or fails, fall back to ask the user in chat:
...
(`/skill:plan-design-review`, `/skill:design-review`) to see what was originally envisioned.
...
Generated by /skill:office-hours on {date}
Branch: {branch}
Repo: {owner/repo}
Status: DRAFT
Mode: Startup
...
Present the reviewed design doc to the user via ask the user in chat:
...
3. Use ask the user in chat to offer opening the resources:
...
- **`/skill:plan-ceo-review`** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
- **`/skill:plan-eng-review`** for well-scoped implementation planning — lock in architecture, tests, edge cases
- **`/skill:plan-design-review`** for visual/UX design review
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"office-hours","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
- **Questions ONE AT A TIME.** Never batch multiple questions into one user question in chat.
