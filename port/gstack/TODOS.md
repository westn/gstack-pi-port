...
**What:** Append structured JSON entry to `.gstack/ship-log.json` at end of every /skill:ship run (version, date, branch, PR URL, review findings, Greptile stats, todos completed, test results).

**Why:** /skill:retro has no structured data about shipping velocity. Ship log enables: PRs-per-week trending, review finding rates, Greptile signal over time, test suite growth.

**Context:** /skill:retro already reads greptile-history.md — same pattern. Eval persistence (eval-store.ts) shows the JSON append pattern exists in the codebase. ~15 lines in ship template.
...
**What:** /skill:ship Step 7.5: screenshot key pages after push, embed in PR body.
...
**What:** /skill:ship and /skill:review post inline review comments at specific file:line locations using `gh api` to create pull request review comments.
...
**What:** /skill:review Step 4.5: browse PR's preview deploy, annotated screenshots of changed pages, compare against production, check responsive layouts, verify accessibility tree.
...
**What:** `/skill:qa` as GitHub Action step, fail PR if health score drops.
...
**What:** After a few runs, check index.md for user's usual tier pick, skip the ask the user in chat.
...
**Why:** Prerequisite for visual PR annotations in /skill:ship and /skill:review.
...
**What:** Create a placeholder resolver in gen-skill-docs.ts encoding the gstack voice guide (friendly, user-forward, lead with benefits). Inject into /skill:ship Step 5, /skill:document-release Step 5, and reference from AGENTS.md.

**Why:** DRY — voice rules currently live inline in 3 places (AGENTS.md CHANGELOG style section, /skill:ship Step 5, /skill:document-release Step 5). When the voice evolves, all three drift.
...
**Context:** Would require logging choices (e.g., append to a JSONL file when ask the user in chat resolves), parsing them, and displaying trends. Similar pattern to eval persistence.
...
- /skill:qa SKILL.md with 6-phase workflow, 3 modes (full/quick/regression)
...
- Cookie picker web UI, /skill:setup-browser-cookies skill
