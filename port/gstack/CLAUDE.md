...
tests via `pi --mode json -p`. Both must pass before creating a PR.
...
│   └── skill-e2e-*.test.ts       # Tier 2: E2E via pi --mode json -p (~$3.85/run, split by category)
...
├── autoplan/        # /skill:autoplan skill (auto-review pipeline: CEO → design → eng)
├── benchmark/       # /skill:benchmark skill (performance regression detection)
├── canary/          # /skill:canary skill (post-deploy monitoring loop)
├── codex/           # /skill:codex skill (multi-AI second opinion via OpenAI Codex CLI)
├── land-and-deploy/ # /skill:land-and-deploy skill (merge → deploy → canary verify)
├── office-hours/    # /skill:office-hours skill (YC Office Hours — startup diagnostic + builder brainstorm)
├── investigate/     # /skill:investigate skill (systematic root-cause debugging)
├── retro/           # Retrospective skill (includes /skill:retro global cross-project mode)
...
1. **Read AGENTS.md** for project-specific config (test commands, eval commands, etc.)
2. **If missing, ask the user in chat** — let the user tell you or let gstack search the repo
3. **Persist the answer to AGENTS.md** so we never have to ask again
...
these automatically after `./setup` during `/skill:gstack-upgrade`.
...
1500-2000 lines. When `pi --mode json -p` reads a file that large, context bloat causes
