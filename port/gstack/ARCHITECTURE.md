...
gstack gives pi a persistent browser and a set of opinionated workflow skills. The browser is the hard part — everything else is Markdown.
...
1. **Compiled binaries.** `bun build --compile` produces a single ~58MB executable. No `node_modules` at runtime, no `npx`, no PATH configuration. The binary just runs. This matters because gstack installs into `~/.pi/agent/skills/` where users don't expect to manage a Node.js project.
...
The CLI reads this file to find the server. If the file is missing or the server fails an HTTP health check, the CLI spawns a new server. On Windows, PID-based process detection is unreliable in Bun binaries, so the health check (GET /skill:health) is the primary liveness signal on all platforms.
...
When a user runs `pair-agent --client`, the daemon starts an ngrok tunnel so a remote paired agent can drive the browser. Exposing the full daemon surface to the internet (even behind a random ngrok subdomain) meant `/skill:health` leaked the root token on any Origin spoof, and `/cookie-picker` embedded the token into HTML that any caller could fetch.

The fix is **two HTTP listeners**, not one:

- **Local listener** (`127.0.0.1:LOCAL_PORT`) — always bound. Serves bootstrap (`/skill:health` with token delivery), `/cookie-picker`, `/inspector/*`, `/welcome`, `/refs`, the sidebar-agent API, and the full command surface. Never forwarded.
...
ngrok forwards only the tunnel port. The security property comes from **physical port separation**: a tunnel caller cannot reach `/skill:health` or `/cookie-picker` because those paths don't exist on that TCP socket. Header inference (check `x-forwarded-for`, check origin) is unreliable (ngrok header behavior changes; local proxies can add these headers); socket separation isn't.

| Endpoint | Local listener | Tunnel listener | Notes |
|---|---|---|---|
| `GET /skill:health` | public (no token unless headed/extension) | 404 | Token bootstrap for extension happens locally only |
...
This prevents other processes on the same machine from talking to your browse server. The cookie picker UI (`/cookie-picker`) and health check (`/skill:health`) are exempt on the local listener — they're 127.0.0.1-bound and don't execute commands. On the tunnel listener nothing is exempt except `/connect`.
...
| `{{DESIGN_SHOTGUN_LOOP}}` | `resolvers/design.ts` | Shared comparison board feedback loop for /skill:design-shotgun, /skill:plan-design-review, /skill:design-consultation |
| `{{UX_PRINCIPLES}}` | `resolvers/design.ts` | User behavioral foundations (scanning, satisficing, goodwill reservoir, trunk test) for /skill:design-html, /skill:design-shotgun, /skill:design-review, /skill:plan-design-review |
...
| 2 — E2E via `pi --mode json -p` | Spawn real pi session, run each skill, check for errors | ~$3.85 | ~20min |
| 3 — LLM-as-judge | Configured model scores docs on clarity/completeness/actionability | ~$0.15 | ~30s |
...
E2E tests spawn `pi --mode json -p` as a completely independent subprocess — not via the Agent SDK, which can't nest inside pi sessions. The runner:

1. Writes the prompt to a temp file (avoids shell escaping issues)
2. Spawns `sh -c 'cat prompt | pi --mode json -p'`
...
| 2 — E2E via `pi --mode json -p` | Spawn real pi session, run each skill, scan for errors | ~$3.85 | ~20min |
| 3 — LLM-as-judge | Configured model scores docs on clarity/completeness/actionability | ~$0.15 | ~30s |
