...
feature in `/skill:setup-gbrain`. If you ran `/skill:setup-gbrain` and it asked
...
|---|---|---|---|
| pi session JSONL | `transcript` | `~/.pi/projects/*/` | High — full conversations including tool I/O |
...
- **Repos under a `deny` trust policy** (set in `/skill:setup-gbrain` Step 6)
...
- pi provider credentials, OPENAI_API_KEY, GitHub tokens
...
Storage tier depends on your gbrain engine (set during `/skill:setup-gbrain`):
...
For example, when you run `/skill:office-hours`, the model context
...
Run `/skill:setup-gbrain` again. It's idempotent: every step detects existing
...
- **"gbrain CLI missing" in the preamble output** — gbrain isn't on
  your PATH. Run `/skill:setup-gbrain` to install/wire it.
