...
# /skill:design-shotgun: Visual Design Exploration
...
"This is /skill:design-shotgun — your visual brainstorming tool. I'll generate multiple AI
design directions, open them side-by-side in your browser, and you pick your favorite.
You can run /skill:design-shotgun anytime during development to explore design directions for
...
**ask the user in chat with pre-filled context:** Pre-fill what you inferred from the codebase,
...
variant, call `~/.pi/agent/skills/gstack/bin/gstack-taste-update approved <variant-path>`. When they
explicitly reject a variant, call `~/.pi/agent/skills/gstack/bin/gstack-taste-update rejected <variant-path>`.
...
Use ask the user in chat to confirm before spending API credits:
...
After receiving feedback (via HTTP POST or ask the user in chat fallback), output a clear
...
Use ask the user in chat to confirm before saving.
...
If standalone, offer next steps via ask the user in chat:

> "Design direction locked in. What's next?
> A) Iterate more — refine the approved variant with specific feedback
> B) Finalize — generate production Pretext-native HTML/CSS with /skill:design-html
