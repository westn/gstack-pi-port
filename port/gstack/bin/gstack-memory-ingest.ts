 ...
 *
 * Sources walked:
 *   ~/.pi/projects/<encoded-cwd>/<uuid>.jsonl   — pi sessions
...
function* walkClaudeCodeProjects(ctx: WalkContext): Generator<{ path: string; type: MemoryType }> {
  const root = join(HOME, ".pi", "projects");
...
function parseTranscriptJsonl(path: string): ParsedSession | null {
  ...
  // Detect format: Codex `session_meta` or pi `type: user|assistant|tool`
  ...
  if (isCodex) {
    ...
    // pi: look for cwd in first non-queue record
