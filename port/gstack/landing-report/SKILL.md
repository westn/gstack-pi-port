...
description: |
  ...
  WIP work likely to ship soon, and what slot /skill:ship would pick next. No
...
# /skill:landing-report — Version Queue Dashboard
...
`/skill:ship` would land in. This skill is a read-only call into the same
`bin/gstack-next-version` utility `/skill:ship` uses, but with nothing mutating.
...
If you ran /skill:ship right now, you'd claim:
...
1. **If there are collisions in the queue** (two open PRs claim the same version):
   ...
   to rerun /skill:ship to pick up the next free slot."
...
3. **If everything looks clean:**
   "Queue is clean. Next /skill:ship will claim a slot without conflict."
