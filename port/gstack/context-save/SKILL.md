...
description: |
  ...
  "save my work". Pair with /skill:context-restore to resume later.
  Formerly /checkpoint — renamed because pi treats /checkpoint as a
...
# /skill:context-save — Save Working Context
...
can resume without losing a beat via `/skill:context-restore`.
...
- `/skill:context-save` or `/skill:context-save <title>` → **Save**
- `/skill:context-save list` → **List**

If the user provides a title after the command (e.g., `/skill:context-save auth refactor`),
use it as the title. Otherwise, infer a title from the current work.

If the user types `/skill:context-save resume` or `/skill:context-save restore`, tell them:
"Use `/skill:context-restore` instead — save and restore are separate skills now."
...
Restore later with /skill:context-restore.
```
...
If the user passes `--all` (e.g., `/skill:context-save list --all`), show contexts
from **all branches**.
...
`/skill:context-save` to save your current working state."
...
- **Infer, don't interrogate.** Use git state and conversation context to fill in
  the file. Only ask the user in chat if the title genuinely cannot be inferred.
- **This is a gstack skill, not a pi built-in.** When the user types
  `/skill:context-save`, invoke this skill via the Skill tool. The old `/checkpoint`
  name collided with pi's native `/rewind` alias — the rename fixed that.
