...
# /skill:canary — Post-Deploy Visual Monitor
...
## User-invocable
When the user types `/skill:canary`, run this skill.

## Arguments
- `/skill:canary <url>` — monitor a URL for 10 minutes after deploy
- `/skill:canary <url> --duration 5m` — custom monitoring duration (1m to 30m)
- `/skill:canary <url> --baseline` — capture baseline screenshots (run BEFORE deploying)
- `/skill:canary <url> --pages /,/dashboard,/settings` — specify pages to monitor
- `/skill:canary <url> --quick` — single-pass health check (no continuous monitoring)
...
Then STOP and tell the user: "Baseline captured. Deploy your changes, then run `/skill:canary <url>` to monitor."
...
Extract the top 5 internal navigation links from the `links` output. Always include the homepage. Present the page list via ask the user in chat:
...
**If a CRITICAL or HIGH alert is detected**, immediately notify the user via ask the user in chat:
