 ...
 *
 * Order:
 *  1. ~/.pi/agent/skills/gstack/bin/gstack-telemetry-log  (global install)
 *  2. .pi/skills/gstack/bin/gstack-telemetry-log    (symlinked dev)
 *  3. bin/gstack-telemetry-log                          (in-repo dev)
 */
function findTelemetryBinary(): string | null {
  const candidates = [
    path.join(os.homedir(), '.pi', 'skills', 'gstack', 'bin', 'gstack-telemetry-log'),
    path.resolve(process.cwd(), '.pi', 'skills', 'gstack', 'bin', 'gstack-telemetry-log'),
