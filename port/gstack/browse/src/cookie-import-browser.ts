...
export async function importCookiesViaCdp(
  ...
  if (!exePath) {
    throw new CookieImportError(
      `Cannot find ${browser.name} executable. Install it or use /skill:open-gstack-browser.`,
      'not_installed',
    );
  }
  ...
  if (await isBrowserRunning(browser.name)) {
    throw new CookieImportError(
      `${browser.name} is running. Close it first so we can launch headless with your profile, or use /skill:open-gstack-browser to control your real browser directly.`,
      'browser_running',
      'retry',
    );
  }
