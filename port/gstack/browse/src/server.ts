...
function findBrowseBin(): string {
  const candidates = [
    ...
    path.resolve(__dirname, '..', '..', '.pi', 'skills', 'gstack', 'browse', 'dist', 'browse'),
    path.join(process.env.HOME || '', '.pi', 'skills', 'gstack', 'browse', 'dist', 'browse'),
  ];
...
// When the spawning CLI process (e.g. a pi session) exits, this
    ...
    fetch: async (req) => {
      ...
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          ...
          const skillRoot = process.env.GSTACK_SKILL_ROOT || `${homeDir}/.pi/skills/gstack`;
        ...
        return new Response(
          ...
          <body><div class="msg"><div class="gold">◈</div><p>GStack Browser ready.</p><p style="font-size:.85em">Waiting for commands from pi.</p></div></body></html>`,
          { status: 200, headers: { 'Content-Type': 'text/html; charset=utf-8' } }
        );
      }

      // Health check — no auth required, does NOT reset idle timer
      if (url.pathname === '/skill:health') {
        ...
        return new Response(JSON.stringify({
          ...
          tabs: browserManager.getTabCount(),
          // Auth token for extension bootstrap. Safe: /skill:health is localhost-only.
      ...
      // ─── /connect — setup key exchange for /skill:pair-agent ceremony ────
      ...
      // ─── Sidebar endpoints (auth required — token from /skill:health) ────
      ...
      if (url.pathname === '/sidebar-agent/event' && req.method === 'POST') {
        ...
        if (body.piSessionId && sidebarSession && !sidebarSession.piSessionId) {
          sidebarSession.piSessionId = body.piSessionId;
          saveSession();
        }
        return new Response(JSON.stringify({ ok: true }), { status: 200, headers: { 'Content-Type': 'application/json' } });
      }
