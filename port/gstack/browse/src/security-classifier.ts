...
function checkHaikuAvailable(): Promise<boolean> {
  ...
  return new Promise((resolve) => {
    const p = spawn(claude.command, [...pi.argsPrefix, '--version'], { stdio: ['ignore', 'pipe', 'pipe'] });
    let done = false;
...
export async function checkTranscript(params: {
  ...
  return new Promise((resolve) => {
    // CRITICAL: spawn from a project-free CWD. `pi --mode json -p` loads AGENTS.md
    // from its working directory into the prompt context. If it runs in a
    // repo with a prompt-injection-defense AGENTS.md (like gstack itself),
    ...
    // Hard timeout. Measured in v1.5.2.0 bench: `pi --mode json -p --model
