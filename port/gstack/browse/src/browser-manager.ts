    ...
    try {
      ...
      if (fs.existsSync(chromePlist)) {
        ...
        const iconCandidates = [
          ...
          path.join(process.env.HOME || '', '.pi', 'skills', 'gstack', 'scripts', 'app', 'icon.icns'), // global install
        ];
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
        // Auth token is served via /skill:health endpoint now (no file write needed).
        // Extension reads token from /skill:health on connect.
