...
## User-invocable
When the user types `/skill:health`, run this skill.
...
After auto-detection, present the detected tools via ask the user in chat:
...
or misconfigured gbrain doesn't stall the entire /skill:health dashboard.
...
6. **Trends require history.** On first run, say "First health check -- no trend data yet. Run /skill:health again after making changes to track progress."
