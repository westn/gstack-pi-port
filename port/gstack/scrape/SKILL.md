...
# /skill:scrape — pull data from a page
...
2. **Prototype path** (~30s) — no matching skill yet, so drive the page
   with `$B` primitives, return the JSON, and suggest `/skill:skillify` so the
...
The user's request after `/skill:scrape` is the intent. If they did not include
one, ask once:
...
> "/skill:scrape is read-only. For mutating flows, use /automate (browser-skills
...
> "Say /skill:skillify to make this a permanent skill (200ms on next call)."
...
- Do NOT suggest /skill:skillify on a broken prototype.
...
- Auth flows / cookie import (use /skill:setup-browser-cookies first)
...
- Do not embed prose around the JSON in the chat reply unless the user
  asked for an explanation — many `/skill:scrape` callers pipe the output to
  `jq`.
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"scrape","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
