...
Contributor mode turns gstack into a self-improving tool. Enable it and pi
...
gstack generates SKILL.md files for two hosts: **Claude** (`.pi/skills/`) and **Codex** (`.agents/skills/`). Every template change needs to be generated for both.
...
| Paths | `~/.pi/agent/skills/gstack` | `$GSTACK_ROOT` (`.agents/skills/gstack` in a repo, otherwise `~/.codex/skills/gstack`) |
| Hook skills | `hooks:` frontmatter (enforced by Claude) | Inline safety advisory prose (advisory only) |
| `/skill:codex` skill | Included (Claude wraps codex exec) | Excluded (self-referential) |
...
When you run `bin/dev-setup`, it creates symlinks in both `.pi/skills/` and `.agents/skills/` (if applicable), so Codex-compatible agents can discover your dev skills too. The `.agents/` directory is generated at setup time from `.tmpl` templates — it is gitignored and not committed.
