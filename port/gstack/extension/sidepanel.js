 ...
 *
 * Chat tab: two-way messaging with pi via file queue.
...
async function sendMessage() {
  ...
  if (result?.ok) {
    ...
    setTimeout(() => {
      ...
      commandInput.placeholder = 'Message pi...';
    }, 2000);
  }
}
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/skill:connect-chrome').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/skill:connect-chrome'; }, 2000);
  });
});
