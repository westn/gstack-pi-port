/**
 * Terminal sidebar tab — interactive pi PTY in xterm.js.
 *
 * Lifecycle (per plan + codex review):
 *   1. Sidebar opens. Terminal is the default-active tab.
 *   2. Bootstrap card shows "Press any key to start pi."
...
(function () {
  ...
  function setState(next, opts = {}) {
    ...
    switch (next) {
      case STATE.IDLE:
        ...
        hide(els.mount);
        hide(els.ended);
        els.bootstrapStatus.textContent = opts.message || 'Press any key to start pi.';
        break;
  ...
  function forceRestart() {
    ...
    setState(STATE.IDLE, { message: 'Starting pi...' });
    tryAutoConnect();
  }
