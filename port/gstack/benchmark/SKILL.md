...
## User-invocable
When the user types `/skill:benchmark`, run this skill.

## Arguments
- `/skill:benchmark <url>` — full performance audit with baseline comparison
- `/skill:benchmark <url> --baseline` — capture baseline (run before making changes)
- `/skill:benchmark <url> --quick` — single-pass timing check (no baseline needed)
- `/skill:benchmark <url> --pages /,/dashboard,/api/health` — specify pages
- `/skill:benchmark --diff` — benchmark only pages affected by current branch
- `/skill:benchmark --trend` — show performance trends from historical data
...
Same as /skill:canary — auto-discover from navigation or use `--pages`.
