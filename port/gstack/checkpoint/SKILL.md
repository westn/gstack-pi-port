...
# /skill:checkpoint — Save and Resume Working State
...
- `/skill:checkpoint` or `/skill:checkpoint save` → **Save**
- `/skill:checkpoint resume` → **Resume**
- `/skill:checkpoint list` → **List**

If the user provides a title after the command (e.g., `/skill:checkpoint auth refactor`),
...
After presenting the checkpoint, ask via ask the user in chat:
...
If the user passes `--all` (e.g., `/skill:checkpoint list --all`), show checkpoints
from **all branches**.
...
`/skill:checkpoint` to save your current working state."
...
- **Infer, don't interrogate.** Use git state and conversation context to fill in
  the checkpoint. Only ask the user in chat if the title genuinely cannot be inferred.
