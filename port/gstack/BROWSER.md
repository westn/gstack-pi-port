...
new headline: `/skill:scrape <intent>` drives a page once, `/skill:skillify` codifies the
flow into a deterministic Playwright script, and the next `/skill:scrape` on the
...
B=./browse/dist/browse           # or ~/.pi/agent/skills/gstack/browse/dist/browse
...
/skill:scrape latest hacker news stories
/skill:skillify                        # writes ~/.gstack/browser-skills/hn-front/...
/skill:scrape hacker news front page   # second call: 200ms via the codified skill
...
2. [The productivity loop — `/skill:scrape` + `/skill:skillify`](#the-productivity-loop)
...
- **Headless** (default). Daemon runs Chromium with no visible window. Fastest,
  cheapest, what skills like `/skill:qa`, `/skill:design-review`, `/skill:benchmark` use by
  default.
...
2. **Prototype path (~30s)** — no match, agent drives the page with `$B goto`,
   ...
   one-line "say `/skill:skillify`" suggestion.
3. **Mutating-intent refusal** — verbs like *submit*, *click*, *fill* route
   to `/automate` (Phase 2b, P0 in `TODOS.md`). `/skill:scrape` is read-only by
   contract.

### `/skill:skillify`

Codifies the most recent successful `/skill:scrape` prototype into a permanent
...
- **D1 — Provenance guard.** Walks back ≤10 agent turns for a clearly-bounded
  `/skill:scrape` result. Refuses with one specific message if cold. No silent
...
byte-identical to the canonical `browse/src/browse-client.ts`. `/skill:skillify`
...
### Atomic write discipline (`/skill:skillify` D3)
...
When in real-browser mode, `/skill:qa` and `/skill:design-review` automatically skip
...
The Side Panel's primary surface is the **Terminal pane** — a live `pi --mode json -p`
...
unpacked → navigate to `~/.pi/agent/skills/gstack/extension` → pin the
...
```bash
/skill:pair-agent                     # generates a setup key, prints connection instructions
...
- **Local listener** (`127.0.0.1:LOCAL_PORT`). Full command surface. Never
  forwarded by ngrok. Used by your pi, the Side Panel, anything
  on your machine.
...
PTY, can't dispatch arbitrary `/command` calls. `/skill:health` endpoint MUST NOT
surface this token.
...
analysis without dumping the full HTML. Used by `/skill:qa` and `/skill:design-review`
...
│   ├── browser-skill-write.ts   # D3 atomic stage/commit/discard helper for /skill:skillify
...
scrape/SKILL.md.tmpl             # /skill:scrape gstack skill — match-or-prototype entry point
skillify/SKILL.md.tmpl           # /skill:skillify gstack skill — codify last /skill:scrape into permanent skill
```
...
For an agent-written skill: drive the page once with `/skill:scrape <intent>`,
say `/skill:skillify`, accept the proposed name in the approval gate. The skill
lands at `~/.gstack/browser-skills/<name>/` after the test passes.

### Deploying to the active skill

The active skill lives at `~/.pi/agent/skills/gstack/`. After making changes:
...
- [`docs/REMOTE_BROWSER_ACCESS.md`](docs/REMOTE_BROWSER_ACCESS.md) — operator guide for `/skill:pair-agent` (setup keys, scoped tokens, denial log)
...
- [`scrape/SKILL.md`](scrape/SKILL.md) — `/skill:scrape` skill: match-or-prototype data extraction
- [`skillify/SKILL.md`](skillify/SKILL.md) — `/skill:skillify` skill: codify last `/skill:scrape` into permanent skill
