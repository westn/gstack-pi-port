...
**CDP-aware skills:** When in real-browser mode, `/skill:qa` and `/skill:design-review` automatically skip cookie import prompts and headless workarounds.
...
4. **Navigate to the extension folder:** Press **Cmd+Shift+G** in the file picker to open "Go to folder", then paste one of these paths:
   - Global install: `~/.pi/agent/skills/gstack/extension`
...
The Chrome side panel includes a chat interface. Type a message and a child Claude instance executes it in the browser. The sidebar agent has access to `Bash`, `Read`, `Glob`, and `Grep` tools (same as pi, minus `Edit` and `Write` ... read-only by design).

**How it works:**

1. You type a message in the side panel chat
2. The extension POSTs to the local browse server (`/sidebar-command`)
3. The server queues the message and the sidebar-agent process spawns `pi --mode json -p` with your message + the current page context
...
**Session isolation:** Each sidebar session runs in its own git worktree. The sidebar agent won't interfere with your main pi session.
...
2. Import cookies from your real Chrome via `/skill:setup-browser-cookies`
...
- **Unnecessary abstraction**: pi already has a Bash tool. A CLI that prints to stdout is the simplest possible interface.
...
1. Push your branch
2. Pull in the skill directory: `cd ~/.pi/agent/skills/gstack && git pull`
