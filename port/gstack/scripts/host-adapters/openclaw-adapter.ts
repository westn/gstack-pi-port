 ...
 *
 * 1. ask the user in chat → prose instructions (tool call → "ask the user")
...
export function transform(content: string, _config: HostConfig): string {
  ...
  // 1. ask the user in chat references → prose
  result = result.replaceAll('ask the user in chat', 'ask the user directly in chat');
  result = result.replaceAll('Use ask the user in chat', 'Ask the user directly');
  result = result.replaceAll('ask the user in chat', 'ask the user directly');
