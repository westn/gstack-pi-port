...
Before each ask the user in chat, choose \`question_id\` from \`scripts/question-registry.ts\` or \`{skill}-{slug}\`, then run \`${bin}/gstack-question-preference --check "<id>"\`. \`AUTO_DECIDE\` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." \`ASK_NORMALLY\` means ask.
...
Before each ask the user in chat, run: \`${bin}/gstack-question-preference --check "<id>"\`.
...
After each ask the user in chat:
\`\`\`bash
