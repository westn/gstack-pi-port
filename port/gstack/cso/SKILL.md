...
## User-invocable
When the user types `/skill:cso`, run this skill.

## Arguments
- `/skill:cso` — full daily audit (all phases, 8/10 confidence gate)
- `/skill:cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `/skill:cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `/skill:cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `/skill:cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/skill:cso --diff` — branch changes only (combinable with any above)
- `/skill:cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `/skill:cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `/skill:cso --scope auth` — focused audit on a specific domain
...
7. If no external search extension is installed, skip checks that require it and note: "external web search (optional, only if a search extension is installed) unavailable — proceeding with local-only analysis."

## Important: Use the Grep tool for all code searches

The bash blocks throughout this skill show WHAT patterns to search for, not HOW to run them. Use pi's Grep tool (which handles permissions and access correctly) rather than raw bash grep. The bash blocks are illustrative examples — do NOT copy-paste them into a terminal. Do NOT use `| head` to truncate results.
...
Scan installed pi skills for malicious patterns. 36% of published skills have security flaws, 13.4% are outright malicious (Snyk ToxicSkills research).
...
- `pi provider credentials`, `OPENAI_API_KEY`, `env.`, `process.env` (credential access)
...
**Daily mode (default, `/skill:cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/skill:cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
For each candidate finding, launch an independent verification sub-task using an optional Agent tool if installed, otherwise inline. The verifier has fresh context and cannot see the initial scan's reasoning — only the finding itself and the FP filtering rules.
...
**Remediation Roadmap:** For the top 5 findings, present via ask the user in chat:
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"cso","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**This tool is not a substitute for a professional security audit.** /skill:cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /skill:cso as
...
**Always include this disclaimer at the end of every /skill:cso report output.**
