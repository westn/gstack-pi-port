...
version: 1.0.0
description: |
  Configure deployment settings for /skill:land-and-deploy. Detects your deploy
...
# /skill:setup-deploy — Configure Deployment for gstack

You are helping the user configure their deployment so `/skill:land-and-deploy` works
...
After this runs once, `/skill:land-and-deploy` reads AGENTS.md and skips detection entirely.

## User-invocable
When the user types `/skill:setup-deploy`, run this skill.
...
merge to main, Render picks it up automatically. The "deploy wait" in /skill:land-and-deploy
...
Use ask the user in chat to gather the information:
...
```markdown
## Deploy Configuration (configured by /skill:setup-deploy)
- Platform: {platform}
- Production URL: {url}
...
Saved to AGENTS.md. /skill:land-and-deploy will use these settings automatically.

Next steps:
- Run /skill:land-and-deploy to merge and deploy your current PR
- Edit the "## Deploy Configuration" section in AGENTS.md to change settings
- Run /skill:setup-deploy again to reconfigure
```
...
- **Idempotent.** Running /skill:setup-deploy multiple times overwrites the previous config cleanly.
