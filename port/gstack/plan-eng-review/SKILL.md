...
**IRON RULE:** When the coverage audit identifies a REGRESSION — code that previously worked but the diff broke — a regression test is added to the plan as a critical requirement. No ask the user in chat. No skipping. Regressions are the highest-priority test because they prove something broke.
...
After producing the coverage diagram, write a test plan artifact to the project directory so `/skill:qa` and `/skill:qa-only` can consume it as primary test input:
...
This file is consumed by `/skill:qa` and `/skill:qa-only` as primary test input. Include only the information that helps a QA tester know **what to test and where** — not implementation details.
...
For each substantive tension point, ask the user in chat:
...
finding via ask the user in chat and getting explicit approval. This applies even when you
...
Analyze the plan's implementation steps for parallel execution opportunities. This helps the user split work across git worktrees (via pi's Agent tool with `isolation: "worktree"` or parallel workspaces).
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"plan-eng-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Suggest /skill:plan-design-review if UI changes exist and no design review has been run** — detect from the test diagram, architecture review, or any section that touched frontend components, CSS, views, or user-facing interaction flows. If an existing design review's commit hash shows it predates significant changes found in this eng review, note that it may be stale.

**Mention /skill:plan-ceo-review if this is a significant product change and no CEO review exists** — this is a soft suggestion, not a push. CEO review is optional. Only mention it if the plan introduces new user-facing features, changes product direction, or expands scope substantially.
...
**If no additional reviews are needed** (or `skip_eng_review` is `true` in the dashboard config, meaning this eng review was optional): state "All relevant reviews complete. Run /skill:ship when ready."

Use ask the user in chat with only the applicable options:
- **A)** Run /skill:plan-design-review (only if UI scope detected and no design review exists)
- **B)** Run /skill:plan-ceo-review (only if significant product change and no CEO review exists)
- **C)** Ready to implement — run /skill:ship when done

## Unresolved decisions
If the user does not respond to an ask the user in chat or interrupts to move on, note which decisions were left unresolved. At the end of the review, list these as "Unresolved decisions that may bite you later" — never silently default to an option.
