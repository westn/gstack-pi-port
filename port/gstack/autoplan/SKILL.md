...
/skill:autoplan reads the full CEO, design, eng, and DX review skill files from disk and follows
...
feasibility blocker (not a preference), the ask the user in chat framing explicitly
...
- Detect DX scope: grep the plan for developer-facing terms (API, endpoint, REST,
  ...
  package, npm, pip, import, require, SKILL.md, skill template, pi, MCP, agent,
  ...
  on top of) or if an AI agent is the primary user (OpenClaw actions, pi skills,
  MCP servers).
...
`[codex-unavailable]` in the degradation matrix. /skill:autoplan completes with
...
- **Two gates.** The non-auto-decided ask the user in chats are: (1) premise confirmation in Phase 1, and (2) User Challenges — when both models agree the user's stated direction should change. Everything else is auto-decided using the 6 principles.
