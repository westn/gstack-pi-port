...
/skill:autoplan reads the full CEO, design, and eng review skill files from disk and follows
them at full depth — same rigor, same sections, same methodology as running each skill
manually. The only difference: intermediate user-question prompts are auto-decided using
...
ask the user in chat: you do, using the 6 principles, instead of the user.
...
```
# /skill:autoplan Restore Point
Captured: [timestamp] | Branch: [branch] | Commit: [short hash]

## Re-run Instructions
1. Copy "Original Plan State" below back to your plan file
2. Invoke /skill:autoplan
...
`<!-- /skill:autoplan restore point: [RESTORE_PATH] -->`
...
(they are already handled by /skill:autoplan):**
- Preamble (run first)
- ask the user in chat Format
- Completeness Principle — Boil the Lake
- Search Before Building
...
Override: every ask the user in chat → auto-decide using the 6 principles.

**Override rules:**
- Mode selection: SELECTIVE EXPANSION
- Premises: accept reasonable ones (P6), challenge only clearly wrong ones
- **GATE: Present premises to user for confirmation** — this is the ONE ask the user in chat
...
Present as a message, then ask the user in chat:

```
## /skill:autoplan Review Complete
...
**Option handling:**
- A: mark APPROVED, write review logs, suggest /skill:ship
...
On approval, write 3 separate review log entries so /skill:ship's dashboard recognizes them:

```bash
COMMIT=$(git rev-parse --short HEAD 2>/dev/null)
TIMESTAMP=$(date -u +%Y-%m-%dT%H:%M:%SZ)

~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"plan-ceo-review","timestamp":"'"$TIMESTAMP"'","status":"clean","unresolved":0,"critical_gaps":0,"mode":"SELECTIVE_EXPANSION","via":"autoplan","commit":"'"$COMMIT"'"}'

~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"plan-eng-review","timestamp":"'"$TIMESTAMP"'","status":"clean","unresolved":0,"critical_gaps":0,"issues_found":0,"mode":"FULL_REVIEW","via":"autoplan","commit":"'"$COMMIT"'"}'
```

If Phase 2 ran (UI scope):
```bash
~/.pi/agent/skills/gstack/bin/gstack-review-log '{"skill":"plan-design-review","timestamp":"'"$TIMESTAMP"'","status":"clean","unresolved":0,"via":"autoplan","commit":"'"$COMMIT"'"}'
```

Replace field values with actual counts from the review.

Suggest next step: `/skill:ship` when ready to create the PR.

---

## Important Rules

- **Never abort.** The user chose /skill:autoplan. Respect that choice. Surface all taste decisions, never redirect to interactive review.
- **Premises are the one gate.** The only non-auto-decided ask the user in chat is the premise confirmation in Phase 1.
