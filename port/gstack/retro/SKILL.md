...
# /skill:retro — Weekly Engineering Retrospective

Generates a comprehensive engineering retrospective analyzing commit history, work patterns, and code quality metrics. Team-aware: identifies the user running the command, then analyzes every contributor with per-person praise and growth opportunities. Designed for a senior IC/CTO-level builder using pi as a force multiplier.

## User-invocable
When the user types `/skill:retro`, run this skill.

## Arguments
- `/skill:retro` — default: last 7 days
- `/skill:retro 24h` — last 24 hours
- `/skill:retro 14d` — last 14 days
- `/skill:retro 30d` — last 30 days
- `/skill:retro compare` — compare current window vs prior same-length window
- `/skill:retro compare 14d` — compare with explicit window
- `/skill:retro global` — cross-project retro across all AI coding tools (7d default)
- `/skill:retro global 14d` — cross-project retro with explicit window
...
```
Usage: /skill:retro [window | compare | global]
  /skill:retro              — last 7 days (default)
  /skill:retro 24h          — last 24 hours
  /skill:retro 14d          — last 14 days
  /skill:retro 30d          — last 30 days
  /skill:retro compare      — compare this period vs prior period
  /skill:retro compare 14d  — compare with explicit window
  /skill:retro global       — cross-project retro across all AI tools (7d default)
  /skill:retro global 14d   — cross-project retro with explicit window
```
...
```
  EUREKA /skill:office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /skill:plan-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
### Plan Completion
Check review JSONL logs for plan completion data from /skill:ship runs this period:
...
When the user runs `/skill:retro global` (or `/skill:retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
If `total_sessions` is 0, say: "No AI coding sessions found in the last <window>. Try a longer window: `/skill:retro global 30d`" and stop.
...
- Behavioral patterns (e.g., "Codex used exclusively for myapp, pi for everything else")
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built /skill:retro global — cross-project retrospective with AI session discovery"
  not "feat: gstack-global-discover" + "feat: /skill:retro global template".
...
- pi: N sessions across M repos — patterns observed
...
When the user runs `/skill:retro compare` (or `/skill:retro compare 14d`):
