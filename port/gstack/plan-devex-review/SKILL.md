...
# /skill:plan-devex-review: Developer Experience Plan Review
...
- Mentions SKILL.md, skill template, pi, AI agent, MCP → **pi Skill**

If NONE of the above: the plan has no developer-facing surface. Tell the user:
"This plan doesn't appear to have developer-facing surfaces. /skill:plan-devex-review
reviews plans for APIs, CLIs, SDKs, libraries, platforms, and docs. Consider
/skill:plan-eng-review or /skill:plan-design-review instead." Exit gracefully.
...
Then SHOW it to the user via ask the user in chat:
...
3. **ask the user in chat per friction point.** One question per friction point found.
...
7. ask the user in chat if there's a genuine DX choice to resolve
...
- **DX EXPANSION:** After fixing to 10, also ask "What would make this dimension
  ...
  individual opt-in ask the user in chats.
...
**STOP.** ask the user in chat once per issue. Recommend + WHY. Reference the persona.
...
**STOP.** ask the user in chat once per issue. Recommend + WHY.
...
- **Boomerang readiness**: Will /skill:devex-review be able to measure reality vs. plan?

**STOP.** ask the user in chat once per issue. Recommend + WHY.

### Appendix: pi Skill DX Checklist

**Conditional: only run when product type includes "pi skill".**

This is NOT a scored pass. It's a checklist of proven patterns from gstack's own DX.

Load reference: Read the "## pi Skill DX Checklist" section from
`~/.pi/agent/skills/gstack/plan-devex-review/dx-hall-of-fame.md`.

Check each item. For any unchecked item, explain what's missing and suggest the fix.

**STOP.** ask the user in chat for any item that requires a design decision.
...
Follow the ask the user in chat format from the Preamble above. Additional rules for
DX reviews:

* **One issue = one user question in chat.** Never combine multiple issues.
...
ask the user in chat. Never batch. For DX debt: missing error messages, unspecified upgrade
...
If any ask the user in chat goes unanswered, note here. Never silently default.
...
```bash
~/.pi/agent/skills/gstack/bin/gstack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
**Recommend /skill:plan-eng-review if eng review is not skipped globally** — DX issues often
...
**Suggest /skill:plan-design-review if user-facing UI exists** — DX review focuses on
developer-facing surfaces; design review covers end-user-facing UI.

**Recommend /skill:devex-review after implementation** — the boomerang. Plan said TTHW would
be [target from 0C]. Did reality match? Run /skill:devex-review on the live product to find
out. This is where the competitive benchmark pays off: you have a concrete target to
measure against.

Use ask the user in chat with applicable options:
- **A)** Run /skill:plan-eng-review next (required gate)
- **B)** Run /skill:plan-design-review (only if UI scope detected)
- **C)** Ready to implement, run /skill:devex-review after shipping
