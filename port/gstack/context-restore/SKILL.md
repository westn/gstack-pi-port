...
version: 1.0.0
description: |
  Restore working context saved earlier by /skill:context-save. Loads the most recent
  ...
  "pick up where I left off". Pair with /skill:context-save.
  Formerly /checkpoint resume — renamed because pi treats /checkpoint
...
# /skill:context-restore — Restore Saved Working Context
...
intentionally different from `/skill:context-save list`, which defaults to the current
branch. `/skill:context-restore` is for Conductor workspace handoff — a context saved
on one branch can be resumed from another.

**Do NOT filter the candidate set by current branch.** The `list` flow does
that; `/skill:context-restore` does not.
...
- `/skill:context-restore` → load the most recent saved context (any branch)
- `/skill:context-restore <title-fragment-or-number>` → load a specific saved context
- `/skill:context-restore list` → tell the user "Use `/skill:context-save list` — listing
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # context window just listing them. /skill:context-save list handles pagination.
...
After presenting, ask via ask the user in chat:
...
"No saved contexts yet. Run `/skill:context-save` first to save your current working
state, then `/skill:context-restore` will find it."
...
- **This is a gstack skill, not a pi built-in.** When the user types
  `/skill:context-restore`, invoke this skill via the Skill tool.
