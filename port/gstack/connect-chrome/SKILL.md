...
# /skill:connect-chrome — Launch Real Chrome with Side Panel
...
[ -z "$_EXT_PATH" ] && [ -f "$HOME/.pi/agent/skills/gstack/extension/manifest.json" ] && _EXT_PATH="$HOME/.pi/agent/skills/gstack/extension"
...
> it won't interfere with this pi window.
...
> - Run any gstack skill (`/skill:qa`, `/skill:design-review`, `/skill:benchmark`) and watch
...
> - `/skill:qa` runs its full test suite in the visible browser — you see every page
>   load, every click, every assertion
> - `/skill:design-review` takes screenshots in the real browser — same pixels you see
> - `/skill:benchmark` measures performance in the headed browser
